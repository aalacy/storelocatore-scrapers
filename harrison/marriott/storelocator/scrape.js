...
async function get_page_hotels(page){
    ...
    const hotels = await page.evaluate(() => {
        return Array.from(
            document.querySelectorAll("#merch-property-results > div > div > div"
                +"> div > div:nth-child(2) > div:nth-child(2) > div"),
            hotel => {
                //let name = hotel.querySelector("div > div > a.js-hotel-quickview-link > h2 > span");
                let details = hotel.querySelector("div:nth-child(2) > div");
...
async function get_destination_info(browser, href){
    ...
    let hotels = []
    await page.goto('https://www.marriott.com'+href, {waitUntil: 'networkidle0', timeout: 120000}).catch(err=> cosole.log(err));
...
async function get_state_hrefs(browser, url){
    ...
    for (let toggle of toggles) {
        await toggle.click();
    const links = await toggle.$$eval('.m-state-list a', as =>
...
(async () => {
    ...
    const dateStr = ''+date.getFullYear()+'/'+date.getMonth()+'/'+
        date.getDate()+'/'+(new Date().toUTCString().split(' ')[4].substring(0, 2))+
        (new Date().toUTCString().split(' ')[4].substring(3, 5));
    ...
    let hrefs = await get_state_hrefs(browser, 'https://www.marriott.com/hotel-search.mi');
