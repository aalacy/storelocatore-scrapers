...
(async () => {
  ...
  const fileLocation = `${config.General_Settings.filenamePrefix}${
    config.Website_Settings.ShopNSave.filenameBody
  }.csv`;
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      for await (const obj of storeData) {
        if (Object.prototype.hasOwnProperty.call(obj, 'Hours2') && obj.Hours2 !== '') {
          obj.Hours = `${obj.Hours}, ${obj.Hours2}`;
        }

        for await (const property of Object.keys(obj)) {
          if (obj[property] === '') {
      ...
      for await (const obj of gasStation) {
        for await (const property of Object.keys(obj)) {
          ...
          if (!Object.prototype.hasOwnProperty.call(obj, 'Hours')) {
            obj.Hours = 'NO-DATA';
          }
...
const updateShopCSV = async (arrayOfObjects, fileLocation) => {
  for await (const obj of arrayOfObjects) {
    const locatorDomain = 'shopnsavefood.com__api';
    const locationName = obj.Name;
    const streetAddress = obj.Address1;
    const city = obj.City;
    const state = obj.State;
    const zip = obj.Zip;
    const countryCode = 'US';
    const storeNumber = obj.StoreID;
    const phone = obj.Phone;
    const locationType = obj.IsGasStation;
    const naics = 'NO-DATA';
    const latitude = obj.Latitude;
    const longitude = obj.Longitude;
    const hours_of_operation = obj.Hours;
