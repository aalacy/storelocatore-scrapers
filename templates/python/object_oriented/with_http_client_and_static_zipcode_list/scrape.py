...
from sgcrawler.sgcrawler_oop import SgCrawlerUsingHttpAndStaticZip
...
if __name__ == "__main__":
    ...
    class MyCrawler(SgCrawlerUsingHttpAndStaticZip):
        def __init__(self):
            super(MyCrawler, self).__init__(
                ...
                radius=10,  # see input to `sgzip.static.static_zipcode_list`
        ...
        def fetch_raw_using(self, http, zipcode):
            # The `http` argument is `SgRequests` because that's what `make_http()` returns.
            # The `zipcode` argument gives you the next zipcode from the static list.
            # Use `self` to access `crawler_domain()` and `logger()`

            # Example:
            locations = http.get(
                f"http://{self.crawler_domain()}/locations?zipcode={zipcode}"
            ).json()
