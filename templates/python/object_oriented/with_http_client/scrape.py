...
from sgcrawler.sgcrawler_oop import SgCrawlerUsingHttp
...
if __name__ == "__main__":
    ...
    class MyCrawler(SgCrawlerUsingHttp):
        def __init__(self):
        ...
        def make_http(self):
            return SgRequests()  # This is the default;
            # Can also: return SgChrome()

        def fetch_raw_using(self, http):
            # The `http` argument is `SgRequests` because that's what `make_http()` returns.
            # Use `self` to access `crawler_domain()` and `logger()`
            # Example:
            locations = http.get(f"http://{self.crawler_domain()}/locations").json()
