...
function crawlBot(initialClick, secondaryClick) {
  (async () => {
    try {
      ...
      await fs.writeFile(
        'test.csv',
        'locator_domain, location_name, street_address, city, state, zip, country_code, store_number, phone, location_type, naics_code, latitude, longitude, hours_of_operation\n'
      );
      ...
      await page.waitForSelector(
        '#results-pagination > div > div.l-display-inline-block > a:nth-child(5)'
      );

      // Get the pagination and use to loop through each page
      const lastPageString = await page.$eval(
        '#results-pagination > div > div.l-display-inline-block > a:nth-child(5)',
        div => {
      ...
      for (let i = 1; i < lastPage; i++) {
        ...
        const dataPropertyOuter = await page.evaluate(() =>
          Array.from(document.body.querySelectorAll('.js-property-list-container > div'), div => {
        ...
        const dataPropertyInner = await page.evaluate(() =>
          Array.from(
            document.body.querySelectorAll(
              '.js-property-list-container > div > div > div > div.js-hotel-location > div > div.m-hotel-address'
            ),
            div => {
              return {
        ...
        const dataProperty = converter.combineObjArray(
          dataPropertyOuter,
          formattedDataPropertyInner
        );

        // Loop to insert each row into the .CSV -> Possible to rework in future, if necessary
        for (const obj of dataProperty) {
          const locatorDomain = 'marriott.com__search__findHotels.mi';
          const locationName = obj.hotelName;
          const streetAddress = obj.address;
          const { city } = obj;
          const { state } = obj;
          const zip = obj.postalCode;
          const countryCode = obj.country;
          const storeNumber = 'N/A';
          const phone = obj.phoneNumber;
          const locationType = obj.brand;
          const naics = '';
        ...
        await page.waitForSelector(
          '#results-pagination > div > div.l-display-inline-block > a.m-pagination-next'
        );
        await page.click(
          ' #results-pagination > div > div.l-display-inline-block > a.m-pagination-next'
        );
