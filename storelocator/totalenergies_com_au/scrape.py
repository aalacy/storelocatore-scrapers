...
def fetchConcurrentSingle(store, http):
    logger.info(store["store_id"])
    url = f"https://api.woosmap.com/stores/{store['store_id']}?key=mapstore-prod-woos"
    loc = request_with_retries(url, http).json()
    return loc


def fetchConcurrentList(list, http, occurrence=max_workers):
    output = []
    total = len(list)
    ...
    with ThreadPoolExecutor(
        ...
        for result in executor.map(fetchConcurrentSingle, list, [http] * len(list)):
            if result:
                count = count + 1
                if count % reminder == 0:
...
def request_with_retries(url, session):
    ...
    with SgRequests() as http:
        ...
        for b in range(1,160):
            for c in range(1,30):
                ...
                for loc in fetchConcurrentList(stores, http):
                    _ = loc["properties"]
                    addr = _["address"]
