...
class Scraper(Scrape):
    ...
    async def get_locations(self, session, page):
        async with session.get(f'https://order.sweetgreen.com/api/restaurants?page={page}&per=1000') as response:
            response = await response.json()
            self.stores.extend(response['restaurants'])

    async def main(self):
        async with aiohttp.ClientSession() as session:
            await asyncio.gather(*[self.get_locations(session, page) for page in range(0, 2)])
