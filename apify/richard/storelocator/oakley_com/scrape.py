    ...
    def fetch_data(self):
        ...
        for zipcode_search in sgzip.for_radius(125):
            ...
            for store in stores:
                ...
                # Street Address
                street_address = store['addressMap']['streetAddress'] if 'streetAddress' in store['addressMap'].keys() else '<MISSING>'

                # City
                city = store['addressMap']['addressLocality'] if 'addressLocality' in store['addressMap'].keys() else '<MISSING>'

                # State
                state = store['addressMap']['addressRegion'] if 'addressRegion' in store['addressMap'].keys() else '<MISSING>'

                # Zip
                zip_code = store['addressMap']['postalCode'] if 'postalCode' in store['addressMap'].keys() else '<MISSING>'
                ...
                # Country
                country = store['addressMap']['addressCountry'] if 'addressCountry' in store['addressMap'].keys() else '<MISSING>'
