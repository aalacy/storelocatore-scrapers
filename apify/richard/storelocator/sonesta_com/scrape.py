...
def fetch_data():
    ...
    for listing in listings:
        if listing['country'] == 'United States':
            ...
            latitude_list.append(listing['geoCoord']['lat'])
            longitude_list.append(listing['geoCoord']['lon'])
            ...
            wait.until(ec.visibility_of_element_located((By.CSS_SELECTOR, ".layout-max")))

            phone_numbers.append(driver.find_element_by_css_selector('a.footer-primary__number-link').get_attribute('textContent').strip())
            zip_codes.append(driver.find_element_by_css_selector('span.postal-code').get_attribute('textContent').strip())
