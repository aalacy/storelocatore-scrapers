...
def fetch_data():
    ...
    for store in stores:
        ...
        lat = store['lat']
        longt = store['lng']
        hours = ''.join(store.get('open_hours')).replace('[','').replace(']','').replace('}','').replace('{','').replace('"','').strip()
        print(hours)
        ...
        if ccode == 'United States':
            ccode = 'US'
        data.append(
            ['https://lightshade.com/', link, title, street, city, state, pcode, 'US', sid, phone, ltype, lat, longt,
             hours])
