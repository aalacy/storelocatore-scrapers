...
def get_cities(county, retry = 0):
    try:
        response = get_session(retry).get('https://www.leroymerlin.ro/xhr/nomenclature/city', params={'countyId': county['id']}, headers=headers)
...
def fetch_locations(county, city):
    try:
        response = get_session().get('https://www.leroymerlin.ro/xhr/contextualization/store', params={'cityId': city['id']}, headers=headers)
...
def fetch_details(county, city, location, retry = 0):
    try:
        ...
        phone = re.search(r'(\d+)', soup.find('li', {'class': 'row phone'}).text).group(1)
...
def fetch_data():
    with  ThreadPoolExecutor() as executor:
        ...
        locations = []
        futures = [executor.submit(fetch_locations, county, city) for county, city in county_cities]
        for future in as_completed(futures):
            locations.extend(future.result())

        pois = []
        futures = [executor.submit(fetch_details, county, city, location) for county, city, location in locations]
        for future in as_completed(futures):
            poi = future.result()
            if poi:
                pois.append(poi)
