  ...
  const crawler = new Apify.PuppeteerCrawler({
    ...
    handlePageFunction: async ({ request, page }) => {
      if (request.userData.urlType === 'initial') {
        ...
        const locationUrls = urls.filter(e => e.match(/www.signaturestyle.com\/locations\/[a-z][a-z]\/(\w|-)+\/borics-hair-care-(\w|-)+.html/))
      ...
      if (request.userData.urlType === 'detail') {
        if (await page.$(checkAddressExists) !== null) {
          ...
          const poiData = {
            locator_domain: 'signaturestyle_com__brands__borics_html',
            location_name,
            street_address,
            city,
            state,
