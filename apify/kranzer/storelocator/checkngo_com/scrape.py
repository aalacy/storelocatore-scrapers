...
base_url = "https://locations.checkngo.com/index.html"
...
class Scrape(base.Spider):
    ...
    async def _fetch_store(self, session, url):
        async with session.get(url, timeout=60 * 60) as response:
            ...
            i.add_xpath('location_name', '//div[@class="LocationInfo-hero"]//span[@itemprop="name"]//text()', lambda x: '- '.join(x), lambda x: x.strip())
            ...
            i.add_xpath('hours_of_operation', '//table[@class="c-location-hours-details"]//tr[@itemprop="openingHours"]/@content', lambda x: '; '.join(x), lambda x: x.strip())
    ...
    async def _fetch_city(self, session, url):
        async with session.get(url, timeout=60 * 60) as response:
            ...
            stores_urls = [urljoin(base_url, s) for s in sel.xpath('//h3[@class="LocationCard-title"]/a/@href')]
