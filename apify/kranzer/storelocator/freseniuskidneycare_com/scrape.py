...
class Scrape(base.Spider):
    async def _fetch_store(self, session, url):
        try:
            async with session.get(url, timeout=60 * 60) as response:
                ...
                i.add_value('hours_of_operation', result.get('openingHours', []), lambda x: '; '.join(x))
                return i
        except:
            return None
    ...
    def crawl(self):
        ...
        for state in base.us_states:
            ...
            locs = [urljoin(base_url, href) for href in body.xpath('//td[@class="locator-results-item-name"]/a/@href')]
