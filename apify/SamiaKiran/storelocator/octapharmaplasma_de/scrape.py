...
DOMAIN = "https://www.octapharmaplasma.de"
...
def fetch_data():
    if True:
        url = "https://www.octapharmaplasma.de/standorte"
        r = session.get(url, headers=headers)
        soup = BeautifulSoup(r.text, "html.parser")
        loclist = soup.findAll("a", {"class": "octa-image-caption"})
        for loc in loclist:
            page_url = DOMAIN + loc["href"]
            if "www.tmd-blutspende.de" in page_url:
                continue
            log.info(page_url)
