...
def fetch_data():
    if True:
        url = "https://www.erstebank.hu/bin/erstegroup/gemesgapi/locations/gem_site_location_locations-hu-ebh?types=BRANCH&items=25000&page=0&language=hu"
        loclist = session.get(url, headers=headers).json()
        for loc in loclist:
            address = loc["address"]
            street_address = strip_accents(address["street"])
            city = strip_accents(address["city"])
            state = strip_accents(address["state"])
