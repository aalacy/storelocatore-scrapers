...
logger = SgLogSetup().get_logger("montanamikes_com")
...
def fetch_data():
    # Your scraper here
    final_data = []
    url = "https://montanamikes.com/store-locator/"
    r = session.get(url, headers=headers, verify=False)
    soup = BeautifulSoup(r.text, "html.parser")
    linklist = soup.select("a[href*=locations]")
    linklist = [
        temp.get("href")
        for temp in linklist
        if "https://montanamikes.com/locations/" in temp.get("href")
    ]
