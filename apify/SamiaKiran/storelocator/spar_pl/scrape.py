...
def fetch_data():
    if True:
        ...
        for loc in loclist:
            ...
            if not sun :
                sun = MISSING
            hours_of_operation = ' mon '+mon+' tue '+tue+' wed ' +wed+' thu '+thu+' fri '+fri+' sat '+sat+' sun '+sun
            if hours_of_operation=='mon <MISSING> tue <MISSING> wed <MISSING> thu <MISSING> fri <MISSING> sat <MISSING> sun <MISSING>':
                hours_of_operation = MISSING
            phone = soup.find("div", {"class": "flex-x white"}).get_text(separator='|', strip=True).split('|')[-1]
