...
DOMAIN = "https://luxehotels.com/"
MISSING = "<MISSING>"


def fetch_data():
    if True:
        url = "https://www.luxehotels.com/contact"
        r = session.get(url, headers=headers)
        soup = BeautifulSoup(r.text, "html.parser")
        loclist = soup.findAll("div", {"class": "col-lg-5"})
        for loc in loclist:
            location_name = loc.find("h3").text
            log.info(location_name)
            address = (
                loc.find("div", {"class": "contact-details-container"})
