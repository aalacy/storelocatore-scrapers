...
def get_data():
    ...
    response = session.get("https://webster.edu/catalog/current/graduate-catalog/campus-locations.html#.YRbMxIhKi3A").text
    ...
    for tag in name_tags:
        if "^" in tag.text.strip() or "*" in tag.text.strip() or "Webster University at Southwestern Illinois College" in tag.text.strip() or "University Center of Lake County" in tag.text.strip():
...
def scrape():
    field_defs = sp.SimpleScraperPipeline.field_definitions(
        ...
        location_name=sp.MappingField(mapping=["location_name"], part_of_record_identity=True),
