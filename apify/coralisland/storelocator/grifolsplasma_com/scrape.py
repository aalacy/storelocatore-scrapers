...
def fetch_data():
    ...
    link_list = response.xpath('//div[@class="all-centers-grid-block"]//a/@href')
    for link in link_list:
        ...
        if len(detail) > 0:
            ...
            output.append("Pride for Donors. Passion for Patients | GRIFOLS") #location type
            geo_loc = data.split(';f=')[1].split(';var')[0].split(';e=')
            output.append(geo_loc[0].replace('+', '')) #latitude
            output.append(geo_loc[1]) #longitude
            store_hours = eliminate_space(store.xpath('.//div[@class="center-column-2"]//p//text()'))
