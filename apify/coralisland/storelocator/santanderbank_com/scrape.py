...
def parse_detail(store, link):
    ...
    lat = store.xpath('//meta[@itemprop="latitude"]/@content')[0]
    lng = store.xpath('//meta[@itemprop="longitude"]/@content')[0]
    output.append(lat) #latitude
    output.append(lng) #longitude
    raw_hours = store.xpath('.//table[@class="c-location-hours-details"]//tbody')
    if len(raw_hours) > 0:
        output.append(get_value(eliminate_space(raw_hours[0].xpath('.//text()')))) #opening hours
    else:
