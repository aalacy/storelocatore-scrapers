...
def fetch_data():
    ...
    for detail_url in store_list:
        ...
        if 'overview' in detail_url:
            detail = etree.HTML(detail_request.text)
            address = validate(detail.xpath('.//div[contains(@class, "property-address")]//text()'))
            address = parse_address(address)
            phone = detail.xpath('.//div[contains(@class, "property-phone")]')
            if len(phone) > 0:
                phone = validate(phone[0].xpath('.//text()')).replace('-', '')
            else:
                phone = ''
            ...
            detail = json.loads(detail_request.text)
            if 'properties' in detail:
                detail = detail['properties'][0]
                ...
                output.append("La Quinta by Wyndham") #location type
