...
def fetch_data():
    ...
    store_list = session.get(url, headers=headers).json()["value"]
    for store in store_list:
        ...
        if get_value(store["LocationAddress"]["CountryCode"]) == "US":
            ...
            output.append(
                get_value(store["LocationAddress"]["CountryCode"])
            )  # country code
            output.append(get_value(store["LocationId"]))  # store_number
            output.append(get_value(store["LocationPhonePrimary"]))  # phone
            output.append("<MISSING>")  # location type
            output.append(
                get_value(str(store["LocationAddress"]["Latitude"]))
            )  # latitude
            output.append(
                get_value(str(store["LocationAddress"]["Longitude"]))
            )  # longitude
            days_of_week = [
                "Monday",
                "Tuesday",
            ...
            output.append(
                "https://titanmachinery.com/dealer-locator/united-states/"
                + store["UrlName"]
            )
