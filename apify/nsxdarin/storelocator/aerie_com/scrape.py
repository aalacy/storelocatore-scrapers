...
def fetch_data():
    ...
    for loc in locs:
        ...
        if typ == "":
            typ = "American Eagle"
        store = "<MISSING>"
        if "aerie store" in name.lower():
            name = "Aerie Store"
        if "aerie outlet" in name.lower():
            name = "Aerie Outlet"
        if "offline" in name.lower():
            name = "OFFLINE by Aerie Store"
        if city != "" and hours != "CLOSED" and "aerie" in name.lower():
            yield SgRecord(
