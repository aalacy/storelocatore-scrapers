...
logger = SgLogSetup().get_logger("dominos_co_id")
...
def fetch_data():
    ...
    for line in r.iter_lines():
        ...
        if 'Sitemap:' in line and '/index.xml' not in line:
            sms.append(line.split('Sitemap:')[1].strip().replace('\r','').replace('\n',''))
    website = "dominos.co.id"
    typ = "<MISSING>"
    country = "ID"
    for sm in sms:
        logger.info(sm)
    ...
    for loc in locs:
        ...
        for line2 in r2.iter_lines():
            line2 = str(line2.decode('utf-8'))
            if '<title>' in line2:
                name = line2.split('<title>')[1].split('|')[0].strip().replace('&#039;',"'")
