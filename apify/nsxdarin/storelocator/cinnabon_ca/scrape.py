...
def fetch_data():
    ...
    for coord in coords:
        ...
        for item in json.loads(r.content):
            ...
            zc = item['zip']
            state = item['state']
            hours = str(item['hours']).replace('<table role=\"presentation\" class=\"wpsl-opening-hours\"><tr><td>','').replace('<time>',': ').replace('<tr><td>','; ').replace('<td>','').replace('<tr>','').replace('</td>','').replace('</tr>','')
            if store not in locs:
                locs.append(store)
                loc = '<MISSING>'
                hours = hours.replace('</time>','').replace('</table>','')
