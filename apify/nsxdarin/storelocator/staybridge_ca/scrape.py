...
logger = SgLogSetup().get_logger('staybridge_ca')
...
def fetch_data():
    alllocs = []
    urls = ['https://www.ihg.com/staybridge/destinations/us/en/canada-hotels']
    for url in urls:
        states = []
        ...
        for state in states:
            ...
            for city in cities:
                ...
                for line3 in r3.iter_lines(decode_unicode=True):
                    if 'https://www.ihg.com/staybridge/hotels/' in line3 and '{"@context":"https://www.schema.org"' in line3:
                        lurl = line3.split('"url":"')[1].split('"')[0]
                        if lurl not in alllocs:
                            alllocs.append(lurl)
                            website = 'staybridge.ca'
                            typ = 'Hotel'
                            hours = '<MISSING>'
