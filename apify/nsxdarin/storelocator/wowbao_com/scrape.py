    ...
    for item in json.loads(r.content)["results"]["locations"]:
        ...
        if "order.online" in loc:
            try:
                ...
                response_ql = session.post(url_ql, json=payload_ql, headers=headers).json()
                add = response_ql['data']['storepageFeed']['storeHeader']['address']['street']
                zc = re.findall('[A-Z]{2} ([A-Z0-9 ]*)',response_ql['data']['storepageFeed']['storeHeader']['address']['displayAddress'])[0]
                state = re.findall('[A-Z]{2}',response_ql['data']['storepageFeed']['storeHeader']['address']['displayAddress'])[0]
                city = response_ql['data']['storepageFeed']['storeHeader']['address']['city']
                hours = response_ql['data']['storepageFeed']['menuBook']['displayOpenHours']
                ooh = [day.title()+f': {hours}' for day in ['monday','tuesday','wednesday','thursday','friday','saturday','sunday']]
                hours_of_operation =  ', '.join(ooh)
                raw_address = response_ql['data']['storepageFeed']['storeHeader']['address']['displayAddress']
