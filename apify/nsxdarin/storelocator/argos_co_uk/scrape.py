    ...
    for loc in locs:
        ...
        for line2 in r2.iter_lines(decode_unicode=True):
            if '"store":{"store"' in line2:
                store = (
                    line2.split('"store":{"store"')[1].split('"id":"')[1].split('"')[0]
                )
                add = (
                    line2.split('"store":{"store"')[1]
                    .split('"address":"')[1]
                    .split('"')[0]
                )
                name = (
                ...
                lat = (
                    line2.split('"store":{"store"')[1].split('"lat":')[1].split(",")[0]
                )
                lng = (
                    line2.split('"store":{"store"')[1].split('"lng":')[1].split(",")[0]
                )
                city = (
                    line2.split('"store":{"store"')[1]
                    .split('"town":"')[1]
                ...
                try:
                    ...
                    zc = "<MISSING>"
                phone = (
                    line2.split('"store":{"store"')[1].split('"tel":"')[1].split('"')[0]
                )
