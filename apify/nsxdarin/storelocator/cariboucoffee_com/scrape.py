...
def fetch_data():
    ...
    for line in r.iter_lines():
        ...
        if '{"countries":' in line:
            jinfo = line.split("html(),")[1].rsplit(");", 1)[0]
            for item in json.loads(jinfo)["countries"]:
                country = item["name"]
                for cname in item["cities"]:
                    city = cname["name"]
                    for sname in cname["stores"]:
                        name = sname["name"]
                        store = "<MISSING>"
                        loc = "<MISSING>"
                        phone = "<MISSING>"
                        ...
                        murl = str(murl)
                        try:
                            lat = murl.split("q=")[1].split("%2C")[0]
                            lng = murl.split("%2C")[1]
                        except:
                            lat = "<MISSING>"
                            lng = "<MISSING>"
                        hours = "<MISSING>"
                        if "%C2" in lat or "%C2" in lng:
                            lat = "<MISSING>"
                            lng = "<MISSING>"
                        yield [
                            website,
                ...
                for day in days:
                    if '<td class="c-location-hours-details-row-intervals' in day:
                        ...
                        if "Closed" in hrs:
                            ...
                            hrs = (
                                hrs.split('instance-open">')[1].split("<")[0]
                                + "-"
                                + hrs.split('instance-close">')[1].split("<")[0]
                            )
                        if hours == "":
                            hours = dname + ": " + hrs
                        else:
