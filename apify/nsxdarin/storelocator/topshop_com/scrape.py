...
def fetch_data():
    calocs = ["Abbotsford|7 OAKS SHOPPING CENTRE, 32900 SOUTH FRASER WAY|ABBOTSFORD|BC",
            ...
            if '"stores":[' in line:
                ...
                for item in items:
                    if 'brandPrimaryEStoreId' in item:
                        ...
                        lat = '<MISSING>'
                        lng = '<MISSING>'
                        add = item.split('"line1":"')[1].split('"')[0] + ' ' + item.split('"line2":"')[1].split('"')[0]
                        add = add.strip()
                        country = 'US'
                        ...
                        hours = hours + '; Tue: ' + item.split('"tuesday":"')[1].split('"')[0]
                        hours = hours + '; Wed: ' + item.split('"wednesday":"')[1].split('"')[0]
                        hours = hours + '; Thu: ' + item.split('"thursday":"')[1].split('"')[0]
                        hours = hours + '; Fri: ' + item.split('"friday":"')[1].split('"')[0]
                        hours = hours + '; Sat: ' + item.split('"saturday":"')[1].split('"')[0]
                        hours = hours + '; Sun: ' + item.split('"sunday":"')[1].split('"')[0]
                        if '-' not in hours:
                            hours = '<MISSING>'
                        loc = '<MISSING>'
