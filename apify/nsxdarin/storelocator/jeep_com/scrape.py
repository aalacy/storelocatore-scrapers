...
logger = SgLogSetup().get_logger('jeep_com')
...
def fetch_data():
    ...
    for code in sgzip.for_radius(50):
        ...
        url = 'https://www.jeep.com/bdlws/MDLSDealerLocator?brandCode=J&func=SALES&radius=50&resultsPage=1&resultsPerPage=100&zipCode=' + code
        ...
        for line in lines:
            ...
            if 'day"' in line and '"sunday" :' not in line:
                dayname = line.split('"')[1].split('day')[0].title()[:3]
                next(lines)
                next(lines)
                next(lines)
                g = next(lines)
