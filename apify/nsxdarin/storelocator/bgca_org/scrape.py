...
def fetch_data():
    for x in range(10, 20, 2):
        for y in range(-160, -140, 2):
            ...
            url = 'https://bgcaorg-find-a-c-1488560011850.appspot.com//x/v1/clubs/' + lat + '/' + lng + '/500/'
    ...
    for x in range(24, 50):
        for y in range(-126, -66):
            ...
            lng = str(y)
            logger.info((str(lat) + ',' + str(lng)))
            url = 'https://bgcaorg-find-a-c-1488560011850.appspot.com//x/v1/clubs/' + lat + '/' + lng + '/250/'
            r = session.get(url, headers=headers)
            for line in r.iter_lines():
                if '"City": "' in line:
                    typ = '<MISSING>'
