            ...
            for item in items:
                if '"photoUrl":' in item:
                    ...
                    if '"dailyHours":[]' not in item:
                        days = item.split('"dailyHours":[')[1].split(']')[0].split('"weekDayName":"')
                        for day in days:
                            if '"openingTime":"' in day:
                                hrs = day.split('"')[0] + ': ' + day.split('"openingTime":"')[1].split('"')[0].rsplit(':',1)[0] + '-' + day.split('"closingTime":"')[1].split('"')[0].rsplit(':',1)[0]
                                if hours == '':
                                    hours = hrs
                                else:
                                    hours = hours + '; ' + hrs
