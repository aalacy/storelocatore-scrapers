...
def fetch_data():
    locs = []
    url = 'https://www.ihg.com/avidhotels/content/us/en/about/locations#scmisc=nav_locations_va'
    r = session.get(url, headers=headers)
    Found = False
    if r.encoding is None: r.encoding = 'utf-8'
    for line in r.iter_lines(decode_unicode=True):
        if '>United States</div>' in line:
            Found = True
        if Found and '<a href="//www.ihg.com/avidhotels/hotels/us/en/' in line:
