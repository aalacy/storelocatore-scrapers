    ...
    for loc in locs:
        ...
        for line2 in lines:
            ...
            if Found and 'day:' in line2:
                hrs = line2.strip().replace('\r','').replace('\n','').replace('\t','')
                if hours == '':
                    hours = hrs
                else:
                    hours = hours + '; ' + hrs
            ...
            if '<meta property="place:location:latitude"  content="' in line2:
                lng = line2.split('<meta property="place:location:latitude"  content="')[1].split('"')[0]
            if '<meta property="place:location:longitude" content="' in line2:
                lat = line2.split('<meta property="place:location:longitude" content="')[1].split('"')[0]
        if hours == '':
            hours = '<MISSING>'
        if phone == '':
            phone = '<MISSING>'
