...
def fetch_data():
    ...
    for loc in locs:
        ...
        for line2 in r2.iter_lines():
            ...
            if (
                '<span class="open' in line2
                and "hoursAlt" not in line2
                and "document." not in line2
            ):
                hrs = (
                    day
                    + " "
                    + line2.split('<span class="open')[1].split('"')[0]
                    + "-"
                    + line2.split('class="hideifclosed')[1].split('"')[0]
                )
                if "Closed" in hrs:
                    hrs = day + " Closed"
                if hours == "":
