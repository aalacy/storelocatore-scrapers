...
logger = SgLogSetup().get_logger("montanas_ca")
...
def fetch_data():
    ...
    for line in r.iter_lines(decode_unicode=True):
        if '"storeNumber":' in line:
            ...
            locs.append(
                "https://iosapi.montanas.ca/CaraAPI/APIService/getStoreDetails?storeNumber="
                + sid
    ...
    for loc in locs:
        ...
        if purl == "":
            purl = "https://www.montanas.ca/en/locations.html"
        if phone == "":
            phone = "<MISSING>"
        if store != "9999":
            yield [
