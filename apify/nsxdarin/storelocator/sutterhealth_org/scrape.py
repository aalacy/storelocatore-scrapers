...
logger = SgLogSetup().get_logger("sutterhealth_org")
...
def fetch_data():
    ...
    for htyp in types:
        locs = []
        url = (
            "https://www.sutterhealth.org/location-search?location-type="
            + htyp
            + "&location-city=&location-affiliate=&location-lat=&location-lng=&location-distance=8.07&location-service=&location-auto=&sort-type=&q=&start=1&max=250"
        )
        ...
        for loc in locs:
            ...
            if "Sutter" not in name:
                name = "Sutter " + name
            if "," in name:
                name = name.split(",")[0].strip()
            if "Sutter Care at Home" in name:
