...
def fetch_data():
    ...
    country = "US"
    typ = "<MISSING>"
    payload = [{"operationName":"GetRestaurants",
                        "variables":{"input":{"filter":"NEARBY",
                                              "coordinates":{"userLat":60,
                                                             "userLng":-150,
                                                             "searchRadius":1024000},
                                              "first":500,"status":"OPEN"}},
        ...
        if '"storeId":"' in line:
            ...
            for item in items:
                if '"stateProvince":"' in item:
                    ...
                    add = add.strip()
                    loc = 'https://www.popeyes.com/store-locator/store/' + item.split(',"id":"')[1].split('"')[0]
                    ...
                    try:
                        hours = 'Sunday: ' + hrs.split('"sunOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"sunClose":"')[1].split(':00"')[0]
                    except:
                        hours = 'Sunday: Closed'
                    try:
                        hours = hours + '; Monday: ' + hrs.split('"monOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"monClose":"')[1].split(':00"')[0]
                    except:
                        hours = hours + '; Monday: Closed'
                    try:
                        hours = hours + '; Tuesday: ' + hrs.split('"tueOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"tueClose":"')[1].split(':00"')[0]
                    except:
                        hours = hours + '; Tuesday: Closed'
                    try:
                        hours = hours + '; Wednesday: ' + hrs.split('"wedOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"wedClose":"')[1].split(':00"')[0]
                    except:
                        hours = hours + '; Wednesday: Closed'
                    try:
                        hours = hours + '; Thursday: ' + hrs.split('"thrOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"thrClose":"')[1].split(':00"')[0]
                    except:
                        hours = hours + '; Thursday: Closed'
                    try:
                        hours = hours + '; Friday: ' + hrs.split('"friOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"friClose":"')[1].split(':00"')[0]
                    except:
                        hours = hours + '; Friday: Closed'
                    try:
                        hours = hours + '; Saturday: ' + hrs.split('"satOpen":"')[1].split(':00"')[0] + '-' + hrs.split('"satClose":"')[1].split(':00"')[0]
                    except:
