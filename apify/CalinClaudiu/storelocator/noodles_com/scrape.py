    ...
    for idx1, state_url in enumerate(state_list):
        ...
        for city in city_list:
            if city.find('span').text == '(1)':
                ...
                for loc in loclist:
                     loc = "https://locations.noodles.com/" + loc.find("a")["href"].replace("..","")
...
def fetch_records(session: SgRequests, state: CrawlState) -> Iterable[SgRecord]:
    for next_r in state.request_stack_iter():
        r = session.get(next_r.url.replace('.com//','.com/'), headers=headers, timeout=180)
        ...
        hours_of_operation =  soup.find("table", {"class": "c-location-hours-details"}).get_text(separator='|', strip=True).replace('|'," ")
