...
def get_storeCA(url):
    ...
    try:
        k['address'] = data.find('li',{'class':'store__loc-address'}).find('span',{'class':'text'}).text
    ...
    try:
        k['city'] = data.find('li',{'class':'store__loc-address'}).find('span',{'class':'text'}).text
        k['city'] = k['city'].split(',')[-2]
    except:
        k['city'] = "<MISSING>"
        
    try:
        k['state'] = data.find('li',{'class':'store__loc-address'}).find('span',{'class':'text'}).text
        k['state'] = k['state'].split(',')[-1]
    except:
        k['state'] = "<MISSING>"
        
    try:
        k['zip'] = data.find('li',{'class':'store__loc-address'}).find('span',{'class':'text'}).text
    ...
    try:
        k['phone'] = data.find('a',{'class':'phone'}).find('span',{'class':'text'}).text
    except:
        k['phone'] = "<MISSING>"
    ...
    try:
        j = []
        k['hours'] = data.find('div',{'class':'store__hours'}).find('ul',{'data-expandable-area':True,'data-expandable':True})
...
def fetch_data():
    ...
    session = SgRequests()
    logzilla.info(f'Generating request links')
    son = session.get(url, headers = headers)
    soup = BeautifulSoup(son.text,'lxml')
    gen = soup.find_all('li',{'class':'store-locator__stores-list-item'})
    links = []
    for i in gen:
        tex = i.find('button',{'class':'store-locator__stores-button'}).find('span',{'class':'text'}).text
        if 'Canada' in tex or 'United States' in tex:
            links.append([j['href'] for j in i.find('div',{'class':'store-locator__stores-details'}).find_all('a',{'class':'store-locator__stores-details-name'})])
...
def scrape():
    ...
    pipeline = SimpleScraperPipeline(scraper_name='armaniexchange.com',
