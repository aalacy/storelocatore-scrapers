...
def fetch_subpage(tup):
    ...
    if k["url"] != None:
        ...
        try:
            ...
            days = list(
                containers[0].find("div").find("div").find("p").stripped_strings
            )
            hours = list(
                containers[1].find("div").find("div").find("p").stripped_strings
            )
...
def fetch_data():
    ...
    lize = utils.parallelize(
        search_space=[
            [dex, son["locations"][dex]["website"]]
            for dex, i in enumerate(son["locations"])
        ],
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        street_address=MultiMappingField(
            mapping=[["data", "address_line_1"], ["data", "address_line_2"]],
        ...
        zipcode=MappingField(
            mapping=["data", "postal_code"],
            value_transform=lambda x: x.replace("None", "<MISSING>"),
            is_required=False,
        ),
        country_code=MappingField(
            mapping=["data", "country"],
            value_transform=lambda x: x.replace("None", "<MISSING>"),
            is_required=False,
        ),
        phone=MappingField(
