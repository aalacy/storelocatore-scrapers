...
def stubborn_store(url):
    ...
    son = SgRequests.raise_on_err(
        session.get("https://liquor.sobeys.com/stores/" + url, headers=headers)
    )
...
def para(idey):
    url = "https://liquor.sobeys.com/wp-json/wp/v2/store/" + idey
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.183 Safari/537.36"
    }
    session = SgRequests()
...
def fetch_data():
    ...
    url = "https://liquor.sobeys.com/store-locator/"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.183 Safari/537.36"
    }
    session = SgRequests()
    ...
    url = "https:/liquor.sobeys.com/wp-json/wp/v2/store/"
    j = utils.parallelize(
        search_space=links,
