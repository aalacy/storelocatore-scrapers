...
def html_hours(x):
    soup = b4(x,'lxml')
    markers = ["PM","AM","A.M","P.M","Mon","Sat","Sun","Tue","Wed","Thu","Fri","y:"]
    unwanted = ["APPOINTMENTS","ustomers","items","call","availability","Note:","year","Appointments", "HALLOWEEN","enforcing"]
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        street_address=MultiMappingField(mapping=[["street"],["streettwo"]], multi_mapping_concat_with = ",", value_transform = fix_comma),
        city=MappingField(mapping=["city"]),
        state=MappingField(mapping=["state"], value_transform = lambda x : x.replace('Wisconsin',"WI")),
        zipcode=MappingField(mapping=["zip"]),
        country_code=MissingField(),
        phone=MappingField(mapping=["tel"] , value_transform = lambda x : '<MISSING>' if 'losed' in x else x),
    ...
    pipeline = SimpleScraperPipeline(scraper_name='pipeline',
