...
def fetch_data():
    ...
    with SgChrome() as driver:
        ...
        time.sleep(5)
        footer = WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.XPATH, '//*[@id="sls-locations-list-scroller"]/footer/nav')))
        ...
        driver.execute_script("return arguments[0].scrollIntoView(true);", footer)
        time.sleep(5)
        logzilla.info(f'Looking for API calls')
        for r in driver.requests:
            if 'store-locator-next' in r.path:
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        page_url=MappingField(mapping=['properties','slug'], value_transform = lambda x : url+x),
        ...
        store_number=MappingField(mapping=['properties','id'], part_of_record_identity = True),
        hours_of_operation=MappingField(mapping=['properties','hoursOfOperation'], value_transform = pretty_hours),
        location_type=MappingField(mapping=['properties','isPermanentlyClosed'], value_transform = lambda x : "<MISSING>" if x == 'False' else "isPermanentlyClosed")
    )

    pipeline = SimpleScraperPipeline(scraper_name='sleepoutfitters.com',
