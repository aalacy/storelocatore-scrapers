...
def fetch_data():
    ...
    data = script.split('<li class="parent"><a href="/Contact-Us/">Contact Us</a></li>')[1]
    ...
    issues = []
    url = 'https://www.securitasinc.com'
    logzilla.info(f'Grabbing cities')
    for i in states:
        ...
        try:
            for j in links.find_all('a',{'class':True,'href':lambda x : x and x.startswith(i)}):
                cities.append(j['href'])
        except:
            issues.append(i)
            logzilla.info(f'Had an issue with state: {i.rsplit("/",2)[-2]} and url:\n{url+i}')
...
def scrape():
    ...
    pipeline = SimpleScraperPipeline(scraper_name='securitasinc.com',
