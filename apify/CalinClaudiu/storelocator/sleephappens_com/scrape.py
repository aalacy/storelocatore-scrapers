...
def fetch_data():
    ...
    for i in soup:
        if 'jqLocator(document).ready(function()' in i.text:
            son = json.loads(str(i.text.split('location_data',1)[1].split('=',1)[1].split('searchcenter',1)[0].rsplit(';',1)[0]))
            for i in son:
                try:
                    i['spe'] = "https://stores.sleephappens.com/"+i['countrycode']+'/'+i['state']+'/'+i['city']+'/'+i['slug']+'/'+i['id']+'/11139'
                    i['trey'] = i['countrycode']
                except:
                    i['spe'] = "https://stores.sleephappens.com/"+i['country']+'/'+i['state']+'/'+i['city']+'/'+i['slug']+'/'+i['id']+'/11139'
                    i['trey'] = i['country']
                yield i
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        hours_of_operation=MappingField(mapping=['hours'], value_transform = pretty_hours),
        location_type=MissingField()
    )

    pipeline = SimpleScraperPipeline(scraper_name='stores.sleephappens.com',
