...
def fetch_data():
    ...
    with SgFirefox() as driver:
        ...
        items =  WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.XPATH,'//*[@id="root"]/div/main/div/div[2]/div')))
        ...
        for i in states:
            ...
            logzilla.info(f'Waiting for page to load')
            stores =  WebDriverWait(driver, 20).until(EC.visibility_of_element_located((By.XPATH,'//*[@id="root"]/div/main/div/div[1]/div[2]/div/a')))
            for r in driver.requests:
                if ('/api/vendors/search/'+i['code']) in r.path:
                    data = json.loads(r.response.body)
                    logzilla.info(f'Found {len(data["vendor-search-results"])} stores in {i["name"]}.')
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        page_url=MappingField(mapping=['slug'], value_transform = lambda x : 'https://order.pieology.com/menu/'+str(x)+'?showInfoModal=true'),
        ...
        hours_of_operation=MappingField(mapping=['weeklySchedule','calendars',0,'schedule'], raw_value_transform = pretty_hours),
        location_type=MappingField(mapping=['unavailableMessage'], is_required = False)
    )

    pipeline = SimpleScraperPipeline(scraper_name='pieology.com',
