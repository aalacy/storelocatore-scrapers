...
def para(url):
    ...
    try:
        k['Latitude'] = data.find('div',{'class':'office-map'}).find('img',{'src':True,'class':'img-responsive'})['src']
    ...
    try:
        k['Longitude'] = data.find('div',{'class':'office-map'}).find('img',{'src':True,'class':'img-responsive'})['src']
        k['Longitude'] = k['Longitude'].split('center=')[1].split('&',1)[0].split(',')[1]
        empty = False
    except:
        k['Longitude'] = '<MISSING>'
...
def fetch_data():
    ...
    soup = b4(soup,'lxml')
    data = soup.find_all('div',{'class':lambda x : x and all(i in x for i in ['sub-pages','dropdown-menu'])})
    pages = ''
    for i in data:
        if 'Contact' in i.text:
            pages = i
        
    data = pages.find_all('a',{'href': lambda x : x and x.startswith('/contact-us-offices/')})
    for i in data:
        ...
        if str(url+i).count('/')>5:
            ...
            if not k['Error']:
                ...
                if str(url+i).count('/')>5:
                    ...
                    logzilla.info(f'(Likely because page has no data)')
                    logzilla.info(f'=!!!!!!!!!!!!!!!!!!!!!!!!!!=')
                    logzilla.info(f'============================\n\n')
            
    known_issues = ['https://www.securitasinc.com/Contact-Us/District-of-Columbia/',
    ...
    for i in final_issues:
        ...
        for j in known_issues:
            if i == j:
                damn = False
        if damn:
            raise Exception("Found an unexpected issue with url:",i,'If this is a non-issue please add the URL to the "known_issues" list:',known_issues)
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        street_address=MappingField(mapping=['Address'], value_transform = fix_comma, part_of_record_identity = True),
        ...
        zipcode=MappingField(mapping=['Zip'], value_transform = lambda x : x.replace('None','<MISSING>'), is_required = False , part_of_record_identity = True),
        country_code=ConstantField('CA'),
        phone=MappingField(mapping=['Phone'], value_transform = lambda x : x.replace('None','<MISSING>') , is_required = False, part_of_record_identity = True),
    ...
    pipeline = SimpleScraperPipeline(scraper_name='securitas.ca',
