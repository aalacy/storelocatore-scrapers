...
def para(url):
    ...
    try:
        k['Address'] = data.find('span',{'itemprop':'streetAddress'}).text.strip()
    except:
        k['Address'] = '<MISSING>'
...
def fetch_data():
    ...
    identities = set()
    for i in states:
        logzilla.info(f'Total stores found: {len(offices)}\n')
        ...
        if count>0:
            stores = soup.find_all('a',{'class':'office-name','href':True})
            for j in stores:
                if j['href'] not in identities:
                    identities.add(j['href'])
                    offices.append('https://www.century21.com'+j['href'])
                


    lize = utils.parallelize(
                    search_space = offices,
...
def scrape():
    ...
    pipeline = SimpleScraperPipeline(scraper_name='century21.com',
