...
def fetch_data():
    ...
    with SgChrome() as driver:
        ...
        driver.get(url)
        #json location
        k = WebDriverWait(driver, 120).until(
                EC.presence_of_element_located(
                    (By.XPATH, '//*[@id="bottom"]/script')
                )
        ...
        session = SgRequests()
        for i in son['stores']:
            soup = session.get(str("https://locations.geisinger.org/details.cfm?id="+str(i["CLINICID"])))
            
            soup = b4(soup.text , "lxml")
            try:
                i['hours']='; '.join(list(soup.find('div',{"class":"officeHours"}).find('p').stripped_strings))
            except Exception:
                try:
                    i['hours']='; '.join(list(soup.find('div',{"class":"officeHours"}).stripped_strings))
                except Exception:
                    try:
                        i['hours']=soup.find('div',{"class":"officeHours"}).text.strip()
                    except Exception:
                        i['hours'] = '<MISSING>'
...
def scrape():
    ...
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        page_url=MappingField(mapping=['CLINICID'], value_transform = lambda x : "https://locations.geisinger.org/details.cfm?id=" + str(x)),
        ...
        zipcode=MappingField(mapping=['ZIPCODE'], value_transform = lambda x : x.replace(' ','')),
        ...
        location_type=MappingField(mapping=['OTHERSERVICES'], value_transform = parse_features, is_required = False)
    )

    pipeline = SimpleScraperPipeline(scraper_name='scr',
