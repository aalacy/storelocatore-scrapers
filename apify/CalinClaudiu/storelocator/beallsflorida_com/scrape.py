...
def fetch_data():
    ...
    url = "https://stores.beallsflorida.com/index.m.html"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36"  # noqa
    }
    ...
    for i in soup.find("div", {"class": "no-results"}).find_all(
        ...
        count = i["href"].count("/")
        if count == 5:
            if str(i["href"].replace("index.m.html", "")) not in cities:
                cities.append(str(i["href"].replace("index.m.html", "")))
        elif count == 4:
            if str(i["href"].replace("index.m.html", "")) not in states:
                states.append(str(i["href"].replace("index.m.html", "")))
        else:
            logzilla.info(f'URL BELOW\n{i["href"]}\nABOVE')
            raise Exception(
                "Something very unexpected happened with the link above"
            )  # noqa
    ...
    for i in states:
        ...
        for j in soup.find("div", {"class": "map-list"}).find_all(
            ...
            if str(i["href"]) not in cities:
                cities.append(str(i["href"]))
                do += 1
...
def scrape():
    url = "https://www.beallsflorida.com/"
    field_defs = SimpleScraperPipeline.field_definitions(
        ...
        city=MappingField(
            mapping=["city"],
            value_transform=lambda x: x.replace("None", "<MISSING>"),  # noqa
        ),
        state=MappingField(
            mapping=["region"],
            value_transform=lambda x: x.replace("None", "<MISSING>"),  # noqa
    ...
    pipeline = SimpleScraperPipeline(
        scraper_name="beallsflorida.com",
