...
def para(url):
    ...
    try:
        k["phone"] = soup.find('a',{'href':lambda x : x and x.startswith('tel:')}).text.strip()
    except Exception:
        k["phone"] = "<MISSING>"
        
    try:
        h = soup.find('div',{'class':'divLDTHours'}).find_all('meta',{'itemprop':'openingHours','content':True})
        k["hours"] = []
        for i in h:
...
def fetch_data():
    ...
    soup = b4(session.get(url,headers = headers).text, 'lxml')
    pages = []
    links = soup.find_all('a',{'href':lambda x : x and ('Locations/US/' in x or 'Locations/CA/' in x)})
    for i in links:
...
def scrape():
    ...
    field_defs = sp.SimpleScraperPipeline.field_definitions(
        ...
        country_code= sp.MappingField(mapping=["page_url"], value_transform = lambda x : x.split('Locations/',1)[1].split('/',1)[0]),
        phone= sp.MappingField(mapping=["phone"], is_required = False),
        store_number= sp.MappingField(mapping=["page_url"], value_transform = lambda x : x.split('/')[-1]),
        hours_of_operation= sp.MappingField(mapping=["hours"], is_required = False),
        location_type= sp.MissingField(),
        raw_address = sp.MissingField(),
        )

    pipeline = sp.SimpleScraperPipeline(scraper_name='pipeline',
