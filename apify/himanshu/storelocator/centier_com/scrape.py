    ...
    for idx, val in enumerate(r["locationDetails"]):
        if idx > 0:
            if val != "":
                ...
                page_url = (
                    locator_domain + soup.find("a", {"title": "More Info"})["href"]
                )
                street_address = list(
                    soup.find("p", {"class": "basic-details"}).stripped_strings
                )[2]
                city = list(
                    soup.find("p", {"class": "basic-details"}).stripped_strings
                )[3].split(",")[0]
                state = (
                    list(soup.find("p", {"class": "basic-details"}).stripped_strings)[3]
                    .split(",")[1]
                    .strip()
                    .split(" ")[0]
                )
                ...
                phone = (
                    list(soup.find("p", {"class": "basic-details"}).stripped_strings)[5]
                    .replace("Phone:", "")
                    .strip()
                )

                hours_of_operation = (
                    " ".join(list(soup.find_all("p")[1].stripped_strings))
                    .replace("Hours: Lobby:", "")
                    .replace("(", "")
                    .replace(")", "")
                    .strip()
                ...
                if hours_of_operation.find("Drive-Up:") != -1:
                    hours_of_operation = hours_of_operation.split("Drive-Up:")[
                        0
                    ].strip()
