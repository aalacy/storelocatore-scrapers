...
def fetch_data():
    ...
    r = requests.get("https://www.theshuckinshack.com/locations/",headers=headers)
    ...
    for state in soup.find_all("div",{'class':"location-wrapper"}):
        ...
        for location in state.find_all("a"):
            ...
            if list(location_soup.find("div",{"class":"flex-column-30"}).find_all("p")[0].stripped_strings) == []:
                continue
            address = list(location_soup.find("div",{"class":"flex-column-30"}).find_all("p")[0].stripped_strings)
            phone = list(location_soup.find("div",{"class":"flex-column-30"}).find_all("p")[1].stripped_strings)[0]
            hours = " ".join(list(location_soup.find("div",{"class":"flex-column-30"}).find_all("div")[-1].stripped_strings))
