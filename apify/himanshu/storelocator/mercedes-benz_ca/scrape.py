...
def fetch_data(): 
    ...
    state_code = ['AB', 'BC', 'MB', 'NB', 'NL', 'NT', 'NS', 'NU', 'ON', 'PE', 'QC', 'SK', 'YT']
    for code in state_code:
        
        json_data = session.get("https://nafta-service.mbusa.com/api/dlrsrv/v1/ca/search?count=10000&filter=mbdealer&state="+str(code)).json()['results']
        ...
        for data in json_data:
            ...
            store.append(city)
            store.append(state)
            store.append(zipp.replace('J7J  1S5','J7J 1S5'))   
