...
def fetch_data():
    ...
    for link in soup.find_all("div",{"class":"et_pb_text_inner"}):
        ...
        addr = list(soup1.find_all("div",{"class":"et_pb_text_inner"})[1].stripped_strings)
        if addr==[]:
            if page_url=="https://www.mackenzieriverpizza.com/rapid-city/" or page_url=="https://www.mackenzieriverpizza.com/fairfield/":
                ...
                addr1 = list(soup1.find_all("div",{"class":"et_pb_text_inner"})[0].stripped_strings)
            ...
            location_name = soup1.find("h2",{"class":"et_pb_slide_title"}).text
            addr1 = list(soup1.find_all("div",{"class":"et_pb_text_inner"})[0].stripped_strings)
            hoo = list(soup1.find_all("div",{"class":"et_pb_text_inner"})[1].stripped_strings)
            street_address = addr1[2]
            city = addr1[3].split(",")[0]
            state = addr1[3].split(",")[1].strip().split(" ")[0]
            try:
            ...
            location_name = soup1.find("h1",{"style":"text-align: center;"}).find("span").text
            addr1=addr
            street_address = addr1[2]
            city = addr1[3].split(",")[0]
            state = addr1[3].split(",")[1].strip().split(" ")[0]
        ...
        map_url = soup1.find("iframe")['src'].split("!2m3")[0].split("!2d")[1].split("!3d")
        lat = map_url[1]
        lng = map_url[0]
