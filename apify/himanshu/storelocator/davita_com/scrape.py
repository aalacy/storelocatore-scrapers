...
def fetch_data():
    ...
    for state in states:
        location_url = (
            "https://www.davita.com/api/find-a-center?location="
            + state
        ...
        if r["locations"] is not None:
            for data in r["locations"]:
                ...
                street_address = (
                    data["address"]["address1"] + " " + str(data["address"]["adress2"])
                ).strip()
                ...
                phone = data["phone"]
                page_url = (
                    "https://www.davita.com/locations/"
                    + str(state.lower())
                    + "/"
                    + str(city.lower())
                    + "/"
                    + str(data["address"]["address1"].replace(" ", "-").lower())
                    + "--"
                    + str(store_number)
                )
                store = [
