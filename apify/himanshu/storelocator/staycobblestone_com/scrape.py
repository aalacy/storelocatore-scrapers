...
def fetch_data():
    ...
    for k1 in k:
        for val in k[k1]['properties']:
            ...
            city = k[k1]['properties'][val]['city'].replace("Wisconsin Rapids","")
            ...
            tem_var.append("staycobblestone")
            tem_var.append(latitude if latitude else "<MISSING>")
            tem_var.append(longitude if longitude else "<MISSING>")
            tem_var.append("<MISSING>")
            
            if "Holdrege" in tem_var or "Andrews" in tem_var or "Two Rivers" in tem_var:
                pass
            else:
