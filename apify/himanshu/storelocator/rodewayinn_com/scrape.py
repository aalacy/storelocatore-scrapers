...
def fetch_data():
    ...
    while coords:
        ...
        for store_data1 in data:
            if store_data1['brandCode'] != "RW":
                continue
            result_coords.append((store_data1["lat"],store_data1["lon"]))
            store1 = []
            store1.append("https://mainstaysuites.com")
            page_url = "https://www.choicehotels.com/"+str(store_data1["address"]["subdivision"].lower())+"/"+str(store_data1["address"]["city"].replace(" ","-").lower())+"/"+str(brand[store_data1['brandCode']])+"/"+str(store_data1["id"].lower())
