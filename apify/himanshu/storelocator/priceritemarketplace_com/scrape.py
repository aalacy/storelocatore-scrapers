...
def fetch_data():
    ...
    json_data = session.get("https://storefrontgateway.brands.wakefern.com/api/near/40.8814882/-74.113198/10000/500/stores").json()
    ...
    for val in json_data['items']:
        ...
        store.append(val['countyProvinceState'] if val['countyProvinceState'] else "<MISSING>")
        ...
        store.append(val['location']['latitude'] if val['location']['latitude'] else "<MISSING>")
        store.append(val['location']['longitude'] if val['location']['longitude'] else "<MISSING>")
        store.append(val['openingHours'].replace("\n","") if val['openingHours'] else "<MISSING>")
