...
def fetch_data():
    ...
    for q in range(0,5):
        ...
        for data in response['data']:            
            if "Canada" in data['address']['countryName'] or "United States" in data['address']['countryName']:
                if "Reseller" != data['attributes'][0]['values'][0]['value']:
                    ...
                    hours = " ".join(list(response1.find("div",{"class":re.compile("mb-singleStore__openingColumn")}).stripped_strings)).replace("Opening hours",'').strip()
                    ...
                    #         openTime = q["slots"][0]['openTime']
                    #         closeTime = q["slots"][0]['closeTime']                    
