...
def fetch_data():
	...
	for index,f in enumerate(full):
		...
		if len(full1) != 1:
			...
			hours = ( " ".join(full1[:-1][5:]).replace("or later as needed for patient care",'').replace("(will soon open Tuesday, Thursday, Saturday)",'').replace("or later as needed for patient care",'').replace("Sick call is available","").replace("The Women and Infants Center clinic is  ","").replace("Total Spine is  ","").replace("  other clinic locations vary. For more information, please call 479-463-8740 .",""))
			location_type="Facilities"
			storeNumber='<MISSING>'
			country_code="US"
			latitude="<MISSING>"
			longitude="<MISSING>"
			hours = hours.replace("  other clinic locations vary. For more information, please call 479-463-8740 .","").replace(" Sick call is available",",").replace(" (or until full).","")
	...
	for index,u in enumerate(urls):
		...
		if index==1:
			...
			zipcode = full[2].split(',')[1].strip().split( )[-1]
			state = full[2].split(',')[1].strip().split( )[-2]
			hours = full[-2].replace("open","").replace("Sick call is available","-").replace("The Women and Infants Center clinic is  ","").replace("Total Spine is  ","").replace("  other clinic locations vary. For more information, please call 479-463-8740 .","")
			phone="479-463-5464"
		if index==2:
	...
	for link in prime_urls:
		...
		for i in col:
			...
			if location_name == "Primary Care - Main Sreet":
				phone = "870.741.3592"
			try:
				hours_of_operation = page_soup.find_all("p",text=re.compile('clinic is open'))[0].text.replace("\n",", ").replace("The clinic is open","").replace("Sick call is available ",",").replace("located on the Lincoln Middle School Campus is open ","-").replace(" The school-based clinic located on the Lincoln Middle School Campus is  ",",").replace(" The  Center for Health clinic is  from ",",").replace("  other clinic locations vary. For more information, please call 479-463-8740 .","").replace("Sick call is available ","").strip()
	...
	for dl in urgent:
		...
		location_name = urgent_soup.find("h1",{"class":"o-location-hero__title u-h2"}).text
		addr = list(urgent_soup.find_all("div",{"class":"m-location-panel__section"})[1].find("p",{"class":"m-location-panel__text"}).stripped_strings)
