...
logger = SgLogSetup().get_logger('ralphs_com')
...
def fetch_data():
    ...
    while zip_code:
        ...
        # try:
        r = session.post('https://www.ralphs.com/stores/api/graphql', headers=headers,data=data)
        # except:
        #     continue
        ...
        for key in datas:
            ...
            page_url = "https://www.ralphs.com/stores/details/"+str(key['divisionNumber'])+"/"+str(store_number)
        ...
        for key1 in datas1:
            ...
            if key1['ungroupedFormattedHours']:
                ...
                hours_of_operation =  "<MISSING>"
            page_url = "https://www.ralphs.com/stores/details/"+str(key1['divisionNumber'])+"/"+str(store_number)
            store = []
            store.append(locator_domain if locator_domain else '<MISSING>')
            store.append(location_name if location_name else '<MISSING>')
            store.append(street_address if street_address else '<MISSING>')
