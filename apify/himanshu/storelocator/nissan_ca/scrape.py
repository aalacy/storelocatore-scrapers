...
def fetch_data():
    ...
    for i in range(len(location_list)):
        weekday = {"7":"Sunday","6":"Saturday","5":"Friday","4":"Thusday","3":"Wednesday","2":"Tuesday","1":"Monday"}
        hours_of_operation  =''
        for q in  location_list[i]['openingHours']['regularOpeningHours']:
            days = weekday[str(q['weekDay'])]
            if "openIntervals" in q:
                open1= q['openIntervals'][0]['openFrom']
                close = q['openIntervals'][0]['openUntil']
                hours_of_operation = hours_of_operation + ' '+days+ ' '+ open1 + ' - '+close
            else:
                hours_of_operation = hours_of_operation + ' '+days+ ' Close '
        store_data = location_list[i]
        store = []
        store.append("https://www.nissan.ca")
        # + " " + store_data["address"]["addressLine2"].lower()
        store.append(store_data["name"].lower())
        store.append(store_data["address"]["addressLine1"].lower() )
