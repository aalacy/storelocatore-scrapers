...
def fetch_data():
    ...
    while code:
        ...
        r = requests.post("https://secure.gotwww.com/gotlocations.com/tommy/index.php",headers=headers,data=r_data)
        data_count = 0
        for store_text in r.text.split("L.marker([")[1:]:
            ...
            store_details = BeautifulSoup(store_text.split(".bindPopup(")[1].split(");")[0],"lxml")
            address = store_details.find_all("a")[-1]["href"].split("=")[-1].replace("+"," ")
            ...
            store.append(name)
            store.append(" ".join(address.split(",")[:-4]))
            if store[-1] in addresses:
                continue
            addresses.append(store[-1])
            store.append(address.split(",")[-4])
            store.append(address.split(",")[-3])
            store.append(address.split(",")[-2] if address.split(",")[-2] != " " else "<MISSING>")
