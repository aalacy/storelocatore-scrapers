...
def fetch_data():
    ...
    r = requests.get("https://www.rosewoodhotels.com/en/luxury-hotels-and-resorts",headers=headers)
    return_main_object = []
    soup = BeautifulSoup(r.text,"lxml")
    for link in soup.find("section",{"id":"us-and-canada"}).find_all("a"):
        print(link["href"])
        location_request = requests.get(base_url + link["href"],headers=headers)
        ...
        if location_soup.find("a",{"class":"inline-a location bold gtm-footer"}) == None:
            ...
            geo_request = requests.get(base_url + location_soup.find("a",{"class":"inline-a location bold gtm-footer"})["href"])
