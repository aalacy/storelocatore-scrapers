...
def write_output(data):
    with open('data.csv', mode='w') as output_file:
        ...
        # Header
        writer.writerow(["locator_domain", "location_name", "street_address", "city", "state", "zip", "country_code", "store_number", "phone", "location_type", "latitude", "longitude", "hours_of_operation","page_url","raw_addresss"])
        # Body
        for row in data:
            writer.writerow(row)
...
def fetch_data():
    ...
    for location in soup.find("div").find_all("div",recursive=False):
        ...
        store.append(location_details[1].replace(state,"").replace(store_zip,""))
        yield store
