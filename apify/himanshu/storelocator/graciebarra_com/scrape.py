...
def fetch_data():
    ...
    for store_data in data:
        ...
        store.append(store_data["field_3_raw"]["street"] if store_data["field_3_raw"]["street"] else "<MISSING>")
        ...
        store.append(store_data["field_3_raw"]["city"].split(",")[0] if store_data["field_3_raw"]["city"] else "<MISSING>")
        # splitting street address with city and getting 0 element
        store.append(store_data["field_3_raw"]["state"] if store_data["field_3_raw"]["state"] else "<MISSING>")
        # This was a special case in which websie had miami and florida as state
        if "Florida" in store[-1]:
            store[-1] = "Florida"
        store.append(store_data["field_3_raw"]["zip"] if store_data["field_3_raw"]["zip"] else "<MISSING>")
