...
def fetch_data():
    ...
    for location in soup.find("datalist",{"data-country-id":"US"}).find_all("option"):
        ...
        location_list = json.loads((state_soup.find("script").text.split("STL.storeArray = ")[1].split("];")[0] + "]").replace("'",'"'))
        for i in range(len(location_list)):
            ...
            store.append("Carolina Herrera")
            store.append(store_data["latitude"])
            store.append(store_data["longitude"])
            store_hours = store_data["timetable"]
            hours = ""
