...
def fetch_data():
    ...
    for i in json.loads(soup_):
        ...
        street_address = re.sub(r"\s+", " ", i['address']).replace(", Hadley MA 01035",'').replace(", North Conway, NH 03860",'').replace(", North Conway, NH 03860",'').replace("Peterborough NH 03458",'').replace(" NY 12946",'').replace(", Warwick RI 02889",'')
        city = i['slug']
        ...
        for h in hour:
            hours_of_operation = hours_of_operation +" "+ h['day'] +" "+  h['open'] +" - "+ h['closed']
        store = []
        ...
        store.append(street_address.replace("Marlborough, MA, 01752",'').replace(", Freeport ME",'') if street_address else '<Missing>')
