    ...
    for zip in search:
        with SgRequests() as session:
            ...
            for _ in locations:
                ...
                search.found_location_at(get(_, "latitude"), get(_, "longitude"))
                yield SgRecord(
...
if __name__ == "__main__":
    search = DynamicZipSearch(country_codes=["us"], expected_search_radius_miles=100)
    app_key = None
    with SgRequests() as session:
        app_key = (
    ...
    with SgWriter(
        ...
        results = fetch_data(search, app_key)
        for rec in results:
