...
# #################################################################################
    ...
    soup = BeautifulSoup(r.text,"lxml")
    nav = soup.find("div",class_="leftnav").find("ul",class_="parents").find("li",class_="parent")
    for li in nav.find("ul",class_="children").find_all("li",class_="child"):
        ...
        try:
            ...
            longitude = "<MISSING>"
            page_url= a
            phone_list = re.findall(re.compile(".?(\(?\d{3}\D{0,3}\d{3}\D{0,3}\d{4}).?"), str(" ".join(address)))[0]
            if "Hours of Operation" in str(" ".join(address)):
                hours = " ".join(address).split("Hours of Operation")[1].split("Specialties")[0].split("Specialty")[0].strip().split()
