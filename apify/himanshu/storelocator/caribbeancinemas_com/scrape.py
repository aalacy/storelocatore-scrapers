...
def fetch_data():
    ...
    for loc_url in footer.find_all(lambda tag: (tag.name == "a") and "ARUBA" != tag.text and "PANAMA" != tag.text and "TRINIDAD"!=tag.text):
        ...
        try:
            coords = rr_soup.find_all('div',class_='section')[1].find('div',class_='column one-fourth1 column_column').find('iframe')
            if coords != None:
                if len(coords['src'].split('!')) != 1:
                    latitude = coords['src'].split('!')[4].split('.')[0][-2:] + "."+coords['src'].split('!')[4].split('.')[-1]
            ...
            info = rr_soup.find('div',class_='sections_group').find('div',class_='items_group clearfix').find('div',{'id':'cineinfo'})
            ...
            if list_address == []:
                ...
                if "cinestarguadeloupe.com" == c:
                    ...
                    city = c.replace('-cinemas','').replace('-megaplex-8', '').strip()
                # print(city)
    ...
    link = soup.find('nav',{'id':'menu'}).find('li',class_='menu-item menu-item-has-children').find('ul',class_='sub-menu mfn-megamenu mfn-megamenu-5')
    for li in link.find_all('li',class_='menu-item'):
        try:
            if "#MORE" != li.a['href']:
                ...
                coords = soup_loc.find_all('div',class_='section')[1].find('div',class_='column one-fourth1 column_column').find('iframe')
                ...
                info = soup_loc.find('div',class_='sections_group').find('div',class_='items_group clearfix').find('div',{'id':'cineinfo'})
                # location_name = info.h4.text.strip()
                ...
                if len(list_address) >1:
                    phone_list =re.findall(re.compile(".?(\(?\d{3}\D{0,3}\d{3}\D{0,3}\d{4}).?"), str(list_address[-1]))
