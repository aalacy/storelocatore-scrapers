...
def fetch_data():
    ...
    for store_data in location_data:
        if store_data["country"] == "US" or store_data["country"] == "CA":
            ...
            store.append(
                address.replace("&amp;", "&")
                .replace(", San Antonio Tx, 78209", "")
                .replace("RICHFIELD, UTAH 84701", "<MISSING>")
            )
            ...
            if "postal_code" in store_data:
                if "US" in store_data["country"]:
                    try:
                        ...
                        temp_zipp = (
                            store_data["postal_code"].split(" ")[-1].replace("VA", "")
                        )
                    zipp = (
                        temp_zipp[:5]
                        .replace("TX 77", "77056")
            ...
            location_request = session.get(
                "https://placesws.adidas-group.com/API/detail?brand=adidas&method=get&category=store&objectId="
                + str(store_data["id"])
                + "&format=json",
                headers=headers,
            )
            soup1 = BeautifulSoup(location_request.text, "html5lib")
            location_data2 = json.loads(
                str(soup1)
                .split("<body>")[1]
                .split("</body>")[0]
                .replace("<br>", "")
            ...
            store.append(
                location_data["storetype"]
                if "storetype" in location_data
                else "<MISSING>"
            )
            try:
                try:
