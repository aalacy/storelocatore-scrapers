...
def fetch_data():
    ...
    r = requests.get("https://locations.pon-bon.com/locations.json",headers=headers)
    return_main_object = []
    location_data = r.json()["locations"]
    for store_data in location_data:
        if store_data["loc"]["country"] != "US":
            continue
        store = []
