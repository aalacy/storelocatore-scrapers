        ...
        try:
            for city in ct_divs:
                if city.find("a", {"class": "gaq-link"}):
                    ...
                    data = store_soup.find("script", {"type": "application/ld+json"}).text
                    ...
                    country_code = 'US'
                    lat_lng = store_soup.find('a',{"class": "directions"})['href'].split('=')[-1]
                    latitude = lat_lng.split(',')[0]
                    longitude = lat_lng.split(',')[1]
                    new_link = store_soup.find('a',{"class": "btn btn-primary full-width store-info gaq-link"})['href']
                    req3 = session.get(new_link, headers=headers)
                    last_soup = BeautifulSoup(req3.text, 'html5lib')
                    script = last_soup.find('script', {'type':'application/ld+json'}).text 
                    ...
                    [str(i).strip().replace('\n', '').replace('\t', '').replace('\r', '') for i in store]
                    yield store
        except:
            continue
def scrape():
