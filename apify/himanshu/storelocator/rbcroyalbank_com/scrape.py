...
def fetch_data():
    ...
    return_main_object = []
    base_url = "https://www.rbcroyalbank.com/"
    addressess = []
    ...
    while coord:
        ...
        if "markerData[1]" in soup.text:
            script = soup.text.split("markerData[1]=")[1].split("=true;for(var")[0].split("{label:")
            ...
            for i in range(len(script))[1:]:
                ...
                http = "https://maps.rbcroyalbank.com/locator/locationDetails.php?l="+str(id1)
                r1 = request_wrapper(http,'post',headers=headers,data=data)
                if r1 == None:
                    continue
                soup1= BeautifulSoup(r1.text,"lxml")
                
                loc_dat = soup1.find("table",{"width":"380px","border":"0","cellspacing":"0","cellpadding":"2"})
                loc_dat1 = soup1.find("table",{"width":"402px","border":"0","cellspacing":"0","cellpadding":"2"})
                ...
                if loc_dat != None:
                    ...
                    city = city_state_zipp.replace(zipp,"").replace(state,"").replace(",","")
                    phone_list = re.findall(re.compile(".?(\(?\d{3}\D{0,3}\d{3}\D{0,3}\d{4}).?"), str(" ".join(list(loc_dat.stripped_strings))))
                    ...
                    # phone  = list(loc_dat.stripped_strings)[4].replace("transit #:","(416) 974-3940")
