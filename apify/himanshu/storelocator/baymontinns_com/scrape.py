...
def fetch_data():
    ...
    location_url1 = "https://www.wyndhamhotels.com/baymont/locations"
    try:
        r = session.get(location_url1, headers=headers,  allow_redirects=False)
    except Exception as e :
        pass
    soup= BeautifulSoup(r.text,"lxml")
    a = soup.find("div",{"class":"aem-rendered-content"}).find_all("div",{"class":"state-container"})[0:46]
    for y in a:
        ...
        for b in e:
            ...
            if b != [] :
                ...
                phone = h['telephone']              
                store = []
                store.append("https://www.wyndhamhotels.com/baymont")
                store.append(location_name if location_name else "<MISSING>") 
                store.append(street_address.strip().lstrip().rstrip() if street_address else "<MISSING>")
                ...
                store.append("Baymont by Wyndham Hotels")
