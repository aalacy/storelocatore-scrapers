...
def fetch_data():
    ...
    r = requests.get("http://muchasgraciasmexicanfood.com/our-locations/",headers=headers)
                ...
                for store_data in location_soup.find_all("div",{"class":"avaris-pagebuilder-block"}):
                    ...
                    if len(location_details) > 2 and len(location_details) < 6:
                        ...
                        if len(location_details[1].split(",")) == 3:
                            ...
                            store.append(" ".join(location_details[1].split(",")[0].split(" ")[:-1]))
                            store.append(location_details[1].split(",")[0].split(" ")[-1])
