...
def fetch_data():
    ...
    linklist = soup.find('nav',{'id':"primary_nav_wrap"}).find('ul').find('ul').findAll("li")    
    for link in linklist:
        ...
        longt ,lat= location.find('iframe')['src'].split('!2d',1)[1].split('!2m',1)[0].split('!3d',1)
        ...
        for ph in phonelist:
            if city in ph.text:
                ...
                phone = ph.text.split('Phone: ',1)[1].split('Box',1)[0].replace('\n','').strip()
                break
