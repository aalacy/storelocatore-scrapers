...
logger = SgLogSetup().get_logger('mobil_ca')
...
def fetch_data():
    ...
    while coords:
        ...
        for esso in r:
            ...
            store.append("https://www.mobil.ca")
            ...
            if esso['Telephone']:
                phone_list = re.findall(re.compile(".?(\(?\d{3}\D{0,3}\d{3}\D{0,3}\d{4}).?"), str(esso['Telephone']))
                if phone_list:
            ...
            if esso["StoreAmenities"]:
                loc_list =[]
                for i in esso["StoreAmenities"]:
                    loc_list.append(i["Name"])
                # logger.info(loc_list)
                if "Esso" in str(" ".join(loc_list)):
                    location_type = "Esso"
                elif "Mobil" in str(" ".join(loc_list)):
                    location_type = "Mobil"
                else:
                    # logger.info(loc_list)
                    continue
