...
def fetch_data():
    ...
    r = requests.get(base_url + "/us-en/store-locator/view-all-stores.html",headers=headers)
    soup = BeautifulSoup(r.text,"lxml")
    return_main_object  = []
    for city in soup.find_all("li",{"class":"baa-city"}):
        link = city.find("a")["href"]
        while True:
            city_request = requests.get(base_url + link,headers=headers)
            city_soup = BeautifulSoup(city_request.text,"lxml")
            print(link)
            if city_soup.find("div",{"class":"baa-list-left"}) == None:
                pass
            else:
                location_list = json.loads(city_soup.find("div",{"class":"baa-list-left"})["data-stores"])
                for i in range(len(location_list)):
                    ...
                    store.append("Ermenegildo Zagna")
                    store.append(store_data["latitude"])
                    store.append(store_data["longitude"])
                    store_hours = store_data["openUntil"]
                    hours = ""
                    hours_object = {"1": "Monday","2":"Tuesday","3":"Wednesday","4":"Thursday","5":"Friday","6":"Saturday","7":"Sunday"}
                    for key in store_hours:
                        hours = hours + hours_object[key] + " open time " + store_hours[key]["open"] + " close time " + store_hours[key]["close"] + " " 
