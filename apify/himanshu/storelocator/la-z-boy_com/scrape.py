...
def fetch_data():
    ...
    while zip:
        try:
            ...
            WebDriverWait(driver, 10).until(lambda x: x.find_element_by_xpath("//input[@id='locator']"))
            if temp_zip == "":
                ...
                WebDriverWait(driver, 10).until(lambda x: x.find_element_by_xpath("//input[@value='" + str(temp_zip) +  "']"))
                time.sleep(4)
                try:
                    ...
                    pass
                try:
                    inputElement = driver.find_element_by_xpath("//input[@value='" + str(temp_zip) +  "']")
                    inputElement.clear()
                    inputElement.send_keys(str(zip))
                    inputElement.send_keys(Keys.ENTER)
                except:
                    ...
                    temp_zip = ""
            time.sleep(2)
            WebDriverWait(driver, 10).until(lambda x: x.find_element_by_xpath("//a[text()='Home']"))
            try:
            ...
            for script in soup.find_all("script"):
                if "var __locatorResults" in script.text:
                    ...
                    data = json.loads(script.text.split("var __locatorResults = ")[1])[0]["map"]
                    break
            for store_data in data:
                ...
                if store_data["website"]:
                    ...
                    if hours_soup.find("a",text=re.compile("Store Hours")):
                        hours_details_request = requests.get("https://www.la-z-boy.com" + hours_soup.find("a",text=re.compile("Store Hours"))["href"],headers=headers)
                        hours_details_soup = BeautifulSoup(hours_details_request.text,"lxml")
                        hours = " ".join(list(hours_details_soup.find("ul",{"class":"hours"}).stripped_strings))
