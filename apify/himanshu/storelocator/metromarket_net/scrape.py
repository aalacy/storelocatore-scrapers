...
def parse_hours(store):
    if 'ungroupedFormattedHours' not in store:
        return '<MISSING>'
    hours_of_operation = ""
    for day_hours in store["ungroupedFormattedHours"]:
        hours_of_operation += day_hours["displayName"] +  " = " + day_hours["displayHours"] + "  "
    return hours_of_operation

def parse_store(key, ltype):
    try:
        location_type = key["banner"] + ' ' + ltype
    except:
        location_type = ltype
    ...
    hours_of_operation = parse_hours(key)
    store = []
