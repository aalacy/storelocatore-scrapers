...
def fetch_data():
    ...
    soup = BeautifulSoup(session.post(
        "https://shoprite.com/StoreLocatorSearch", headers=headers, data=payload).text, 'lxml')
    for dt in soup.find_all("li", {"class": "stores__store"}):
        ...
        store_number = json.loads(dt.find(
            "button", {"class": "store__focusResultButton"})['data-json'])['storeId']
        ...
        hours_of_operation = dt.find(
            "p", {"class": "hoursAndServices__schedule storeDetails__content"}).text.strip()
