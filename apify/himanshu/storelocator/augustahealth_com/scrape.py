...
def fetch_data():
	...
	for link  in soup.find_all("div",class_="view-content")[1:]:
		...
		street_address = link.find("div",class_="street-block").text.strip().split("Suite")[0].replace(",",'')
		# if "" in street_address1:
	...
	for loc in soup.find_all("div",class_="location"):
		...
		street_address =" ".join(add[:-1]).split("Suite")[0].replace(",",'')
		city = add[-1].split(",")[0]
	...
	street_address = address[1].split("Suite")[0].replace(",",'')
	...
	r = session.get("https://www.augustahealth.com/laboratory/locations",headers=headers)
	soup = BeautifulSoup(r.text,"lxml")
	for index,div_blue in enumerate(soup.find_all("div",class_="box-rounded-blue")):
		address = list(div_blue.find("p",class_="location").stripped_strings)
		if index==0:
			location_name = div_blue.parent("h3")[0].text
		if index==1:
			location_name = div_blue.parent("h3")[1].text
		if index==2:
			location_name = div_blue.parent("h3")[2].text
		if index==3:
			location_name = div_blue.parent("h3")[3].text		
		street_address = address[0].split("Suite")[0].replace(",",'')
		city = address[-1].split(",")[0]
		state = address[-1].split(",")[-1].split()[0].strip()
		zipp = address[-1].split(",")[-1].split()[-1].strip()
		country_code = "US"
		latitude = div_blue.find("p",class_="location").find("a")["href"].split("@")[1].split(",")[0]
		longitude = div_blue.find("p",class_="location").find("a")["href"].split("@")[1].split(",")[1]
		phone = list(div_blue.find("h4",text=re.compile("Phone")).nextSibling.stripped_strings)[0]
		hours_of_operation = " ".join(list(div_blue.find("table").stripped_strings))
		location_type = "laboratory"
		store_number = "<MISSING>"
		page_url ="https://www.augustahealth.com/laboratory/locations"
		store=[]
