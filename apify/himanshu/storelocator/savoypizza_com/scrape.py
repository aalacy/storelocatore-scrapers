...
def fetch_data():
    ...
    r = requests.get("https://savoypizza.com",headers=headers)
    ...
    for location in soup.find("nav",{"class":"locationsNav"}).find_all("a"):
        ...
        hours = " ".join(list(location_soup.find("div",{'class':"main"}).find_all("p")[1].stripped_strings))
        ...
        store.append("red's savoy pizza")
        store.append(lat)
        store.append(lng)
        store.append(hours)
