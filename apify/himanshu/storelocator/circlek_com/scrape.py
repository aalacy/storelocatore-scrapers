...
def fetch_data():
    ...
    for i in k:
        ...
        if state1 != None:
            ...
            state = "<MISSING>"
        phone1 = soup1.find("div",{"class":"store-info__item store-info__item--phone icon-circlek_icons_telephone"})
        if phone1 != None:
            phone = phone1.text.replace("\n","")
        else:
            phone = "<MISSING>"
        hours_of_operation1 = soup1.find("div",{"class":"columns large-12 middle hours-wrapper"})
        if hours_of_operation1 != None:
            hours_of_operation = " ".join(list(hours_of_operation1.stripped_strings))
        else:
            hours_of_operation = "<MISSING>"
        latitude = k[i]['latitude']
        longitude = k[i]['longitude']
        page_ural =base_url+k[i]['url']
        store = [locator_domain, location_name.encode('ascii', 'ignore').decode('ascii').strip(), street_address.encode('ascii', 'ignore').decode('ascii').strip(), city.encode('ascii', 'ignore').decode('ascii').strip(), state.encode('ascii', 'ignore').decode('ascii').strip(), zipp.encode('ascii', 'ignore').decode('ascii').strip(), country_code,
                store_number, phone.encode('ascii', 'ignore').decode('ascii').strip(), location_type, latitude, longitude, hours_of_operation.replace("hours","").encode('ascii', 'ignore').decode('ascii').strip(),page_ural]
