...
def fetch_data():
    ...
    r = session.get("https://www.genesishealth.com/facilities/location-search-results/", headers=headers)
    ...
    for page in soup.find("div",{"class":"Pagination"}).find_all("option"):
        ...
        for script in soup1.find("div",{"class":"LocationsList"}).find_all("li"):
            ...
            try:
                hours = " ".join(list(location_soup.find("div",{"class":"OfficeHoursContainer"}).stripped_strings)).replace("Hours of Operation:","").strip()
            except:
                hours = "<MISSING>"
