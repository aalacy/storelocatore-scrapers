...
def fetch_data():
    ...
    for store_data in dict_from_json:
        ...
        address = rreplace(store_data["wpcf-yoox-store-address"],store_data["location"]["city"]["name"],"",1)
        street_address = " ".join(address.split("|")[:-1])
        store_zip = address.split("|")[-1].split(",")[0]
        store.append(street_address)
        store.append(store_data["location"]["city"]["name"])
        store.append(address.split("|")[-1].split(",")[-1] if len(address.split("|")[-1].split(",")) == 3 else "<MISSING>")
