...
def fetch_data():
    ...
    soup = bs(requests.get("https://www.booking.com/country.html").content, "lxml")
    ...
    for Accommodation in soup.find("a",text=re.compile("Canada")).parent.parent.parent.find_all("a"):
        ...
        accmd_soup = bs(requests.get(base_url + Accommodation['href']).content, "lxml")

        for region_link in accmd_soup.find("h2",text=re.compile("Top Regions in Canada")).parent.findNext("div").find_all("a"):

            region_soup = bs(requests.get(base_url + region_link['href']).content, "lxml")
            
            region_id = region_soup.find("a",{"title":re.compile("All vacation rentals")})['href'].split("region=")[1].split(";")[0]
            ...
            while True:
                ...
                off_soup = bs(requests.get(off_url).content, "lxml")   

                if off_soup.find("a",{"class":"hotel_name_link url"}) == None:
                    break
                ...
                for url in off_soup.find_all("a",{"class":"hotel_name_link url"}):
                    ...
                    location_soup = bs(requests.get(page_url).content, "lxml")

                    print(location_soup.find(lambda tag: (tag.name == "script") and '"streetAddress"' in tag.text))
                    exit()
