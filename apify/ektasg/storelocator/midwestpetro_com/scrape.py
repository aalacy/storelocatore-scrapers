...
def fetch_data():
    ...
    driver.get("https://www.midwestpetro.com/#locations?all")
    time.sleep(5)
    stores = driver.find_elements_by_css_selector('div.search-row')
    page_url = "https://www.midwestpetro.com/#locations?all"
    for store in stores:
            location_name= store.find_element_by_id('station-name').text
            store_num = location_name.split("#")[1]
            phone = store.find_element_by_css_selector('div.search-result-link.right').text.splitlines()[1]
            loc_type = store.find_element_by_id('brand_logo').get_attribute('alt')
            addr = store.find_element_by_css_selector('div.search-result.four').text.splitlines()
            street_addr = addr[0]
            ...
            if hours_of_op == "":
                hours_of_op = store.find_element_by_css_selector('div.search-result.two > img').get_attribute('alt')
            data.append([
