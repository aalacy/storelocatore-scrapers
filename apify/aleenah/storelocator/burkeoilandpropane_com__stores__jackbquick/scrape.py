...
def fetch_data():
    ...
    divs = soup.find_all('div', {'class': 's_BIwzIGroupSkin'})
    ...
    for div in divs:
        ...
        state=csz[0]
        type=data[3]
        type=type.replace('(','').replace(')','').replace('$','Burke oil payment center').replace('K','Kerosene').replace('G','Gasoline with no ethanol').replace('E','Gasoline containing ethanol').replace('D','Highway diesel')
