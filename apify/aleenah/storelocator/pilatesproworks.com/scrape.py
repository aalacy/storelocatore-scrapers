...
def fetch_data():
    ...
    for url in page_url:
        ...
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        div=soup.find('script', {'data-name': "static-context"})
        lat.append(re.findall(r'"mapLat":(-?[\d\.]*)',div.text)[0])
        long.append(re.findall(r'"mapLng":(-?[\d\.]*)',div.text)[0])
        #print(driver.find_element_by_xpath("/html/body/div[4]/div[1]/section/div[3]/div[2]/div/div/div/div[32]/div[2]/div/div").text)
        #print(soup)
        try:
