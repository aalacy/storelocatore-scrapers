...
def fetch_data():
    ...
    for city_link in city_list:
        ...
        for loc in locs:
            ...
            ## store inside macys
            name_brand = loc.find('span', {'class': 'location-name-brand'}).text
            name_loc = loc.find('span', {'class': 'location-name-geo'}).text
            location_name = name_brand + ' ' + name_loc
            
            hours_json = json.loads(loc.find('div', {'class': 'js-location-hours'})['data-days'])
            hours = ''
            ...
            street_address = loc.find('span', {'class': 'c-address-street-1'}).text
            street_2 = loc.find_all('span', {'class': 'c-address-street-2'})
            if len(street_2) == 2:
                street_address += street_2[1].text
            
            city = loc.find('span', {'class': 'c-address-city'}).text.replace(',', '').strip()
            state = loc.find('abbr', {'class': 'c-address-state'}).text
            zip_code = loc.find('span', {'class': 'c-address-postal-code'}).text
    ...
    for link in link_list:
        ...
        name_loc = soup.find('span', {'class': 'location-name-geo'}).text
        location_name = name_brand + ' ' + name_loc
