...
BASE_URL = "https://branches.halifax.co.uk/"
LOCATION_URL = "https://branches.halifax.co.uk"
HEADERS = {
...
def fetch_store_urls():
    ...
    content = soup.find("section", {"class": "Directory Directory--ace CityList"})
    state_links = content.find_all("a", {"class": "Directory-listLink"})
    for row in state_links:
        count = row["data-count"].replace(")", "").replace("(", "")
        if int(count) > 1:
            data = pull_content(BASE_URL + row["href"])
            links = data.find_all("a", {"class": "Teaser-ctaLink"})
            for link in links:
...
def fetch_data():
    ...
    for page_url in page_urls:
        ...
        zip_code = handle_missing(
            address.find("span", {"itemprop": "postalCode"}).text.strip()
        )
