...
DOMAIN = "moreretail.com"
LOCATION_URL = "https://moreretail.in/store-locator"
API_URL = "https://moreretail.in/get-stores"
HEADERS = {
...
def fetch_data():
    ...
    cities = soup.find("select", id="city").find_all("option")
    for val in cities:
        city = val["value"]
        if not city:
            continue
        ...
        for row in data["stores"]:
            ...
            if not row["map_location"]:
                ...
                latitude, longitude = get_latlong(row["map_location"])
