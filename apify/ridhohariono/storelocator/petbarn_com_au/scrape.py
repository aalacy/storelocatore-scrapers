...
API_URL = "https://www.petbarn.com.au/store-finder/index/dataAjax"
HEADERS = {
...
def get_hoo(hours_content):
    ...
    hours_of_operation = ""
    if hours_content:
        hours = hours_content[-1].get_text(strip=True, separator="@@").split("@@")
        try:
...
def fetch_data():
    ...
    for row in data:
        ...
        phone = row["p"]
        if "Petbarn" in row["m"]:
            location_type = "Petbarn"
        elif "City Farmers" in row["m"]:
            location_type = "City Farmers"
        else:
