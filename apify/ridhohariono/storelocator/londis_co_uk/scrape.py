...
DOMAIN = "londis.co.uk"
BASE_URL = "https://www.londis.co.uk"
LOCATION_URL = "https://www.londis.co.uk/sitemap.xml"
HEADERS = {
...
def find_meta(soup, query):
    ...
    if not el:
        return "<MISSING>"
    return soup.find("meta", query)["content"]
...
def fetch_store_urls():
    ...
    store_urls = []
    for val in soup.find_all("loc", text=re.compile(r"\/our-stores\/\D+")):
        if not re.match(r".*\-0$", val.text):
