...
LOCATION_URL = "https://www.theperfumeshop.com/all-stores"
HEADERS = {
    "Accept": "*/*",
...
def fetch_data():
    ...
    contents = soup.select("a.all_stores__store_info.no-transition")
    for row in contents:
        page_url = BASE_URL + row["href"]
        store = pull_content(page_url).find("main")
        info = json.loads(store.find("script", type="application/ld+json").string)
        ...
        hours_of_operation = (
            store.find("table", {"class": "store_schedule"})
            .get_text(strip=True, separator=",")
            .replace(":,", ": ")
            .strip()
        )
        store_number = MISSING
        latlong = store.find("div", {"class": "store_details__map"})
