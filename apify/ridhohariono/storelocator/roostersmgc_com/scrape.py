...
LOCATION_URL = "https://roostersmgc.com/book-appointment.html"
HEADERS = {
...
def fetch_data():
    ...
    state_locations = soup.select("div.state-buttons a")
    for state_location in state_locations:
        contents = pull_content(BASE_URL + state_location["href"])
        page_urls = contents.select("table a")
        for url in page_urls:
            ...
            hours_of_operation = (
                store.find("div", {"class": "salon-timings"})
                .get_text(strip=True, separator=",")
                .replace("Book Online,", "")
                .replace("Hours:,", "")
                .replace("Hours:", "")
