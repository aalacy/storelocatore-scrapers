...
API_URL = (
    "https://pizzerias.mammaroma.be/wp-content/uploads/ssf-wp-uploads/ssf-data.json"
)
HEADERS = {
...
def fetch_data():
    ...
    data = session.get(API_URL, headers=HEADERS).json()["item"]
    for row in data:
        page_url = row["exturl"]
        ...
        city = addr_split[1].strip()
        state = MISSING
        zip_postal = addr_split[2].strip()
        phone = row["telephone"]
        country_code = "BE"
