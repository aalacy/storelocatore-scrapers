...
LOCATION_URL = "https://vaquitaexpress.com.co/tiendas/"
HEADERS = {
...
def fetch_data():
    ...
    contents = soup.find("script", string=re.compile(r"var stores = .*")).string.strip()
    data = json.loads(
        re.search(r"var stores = .*(\[\{.*\}\])", " ".join(contents.split()))
        .group(1)
        .replace('.png",', '.png"')
    )
    ...
    for row in data:
        ...
        raw_address = row["properties"]["address"].strip()
