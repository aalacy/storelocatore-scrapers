...
DOMAIN = "snydersdrugstore.com"
LOCATION_URL = "https://www.snydersdrugstore.com/locations/"
HEADERS = {
...
def fetch_data():
    ...
    contents = soup.select("a.store-details-link")
    for row in contents:
        page_url = row["href"]
        store = pull_content(page_url)
        location_name = store.find("h2", {"class": "fl-heading"}).text.strip()
        raw_address = store.find("p", {"class": "location-address"}).get_text(
            strip=True, separator=","
        )
        street_address, city, state, zip_postal = getAddress(raw_address)
        phone = store.find("a", {"href": re.compile(r"tel:\D+")}).text.strip()
        country_code = "US"
        hours_of_operation = (
            store.find("p", {"class": "daily-hour"})
