...
def fetch_data():
    ...
    contents = soup.find_all(
        "a", {"role": "menuitem", "href": re.compile(r"\/locations\/\D+")}
    )
    for row in contents:
        page_url = BASE_URL + row["href"]
        store = pull_content(page_url).find("div", {"class": "store-location"})
        ...
        phone = store.find("h3", text="Phone Number").find_next("a").text.strip()
        country_code = "US"
        hours_of_operation = (
