...
def fetch_data():
    ...
    for key, value in links.items():
        if key.startswith('/locations/americas-colocation/united-states-colocation/') or key.startswith('/locations/americas-colocation/canada-colocation/'):
            ...
            if 'contentlets' in _location and _location['contentlets']:
                ...
                state = ''
                zip = ''
                # if page_url == 'https://www.equinix.com/locations/americas-colocation/canada-colocation/calgary-data-centers/cl2/':
                ...
                if 'canada' in location["address"].lower():
                    ...
                    country_code = 'CA'
                    city = _split[-3].strip()
                    state = ''.join(_split[-2].strip().split(' ')[:-2])
                    zip = ' '.join(_split[-2].strip().split(' ')[-2:])
                elif 'canada' in location.get("description", "").lower():
                    street_address = _split[0]
                    country_code = 'CA'
                    city = _split[-2].strip()
                    state = ''.join(_split[-1].strip().split(' ')[:-2])
                    zip = ' '.join(_split[-1].strip().split(' ')[-2:])
                else:

                    if len(_split) == 1:
                        street_address = _split[0].split('\n')[0].strip()
                        city = ''.join(_split[0].split('\n')[-1].split(' ')[:-2]).strip()
                    elif len(_split) == 2:
                        street_address = _split[0].split('\n')[0].replace('<br>', '').strip()
                        ...
                        if not city:
                            if len(_split[1].split('\n')) > 1:
                                city = _split[1].split('\n')[1]
                                street_address = _split[0] +  _split[1].split('\n')[0].replace('<br>', '').strip()
                            else:
                                city = _split[1].strip()
                    try:
