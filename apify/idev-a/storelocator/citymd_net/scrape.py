...
def fetch_data():
    ...
    for link in links:
        ...
        street_address, city, state, zip, country_code = myutil.parse_address_simple(address)
        ...
        direction = soup1.select_one('a.directions-link')['href'].split('/')[-2].split(',')
        latitude = direction[0]
        longitude = direction[1]
        tags = soup1.select_one('h4.margin-bottom-0').find_all_next('p')
        hours_of_operation = '; '.join([tag.text for tag in tags if tag.select_one('span.dow')])
