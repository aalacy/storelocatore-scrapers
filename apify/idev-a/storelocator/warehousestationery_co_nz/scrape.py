...
base_url = "https://www.warehousestationery.co.nz/stores/all-regions"


def fetch_data():
    with SgRequests() as session:
        soup = bs(session.get(base_url, headers=_headers).text, "lxml")
        regions = soup.select("div.view-all-stores-regions ul li a")
        for region in regions:
            region_url = locator_domain + region["href"]
            logger.info(region_url)
            locations = bs(
                session.get(region_url, headers=_headers).text, "lxml"
            ).select("div.store-info")
            for _ in locations:
                page_url = locator_domain + _.select_one("table td a")["href"]
                logger.info(page_url)
                res = session.get(page_url, headers=_headers).text
                ss = json.loads(
                    res.split("var storeModel =")[1]
                    .split("RefApp.Modules")[0]
                    .strip()[:-1]
                )
                ...
                hours = [
                    ...
                    for hh in sp1.select("div.opening-hours table tr")
                ]
                raw_address = " ".join(
