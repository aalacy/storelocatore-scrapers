...
def fetch_data():
    ...
    for b in block:
        ...
        street_address = ''.join(tree.xpath('//div[@class="sub-info contact"]/a[1]/text()[1]')).strip()
        adr = ''.join(tree.xpath('//div[@class="sub-info contact"]/a[1]/text()[2]')).strip()
        phone = ''.join(tree.xpath('//div[@class="sub-info contact"]/a[contains(@href, "tel")]/text()')).strip()
        ...
        hours_of_operation = ' '.join(tree.xpath('//h6[contains(text(), "HOURS")]/following-sibling::p/text()')).replace('\n','').strip()
