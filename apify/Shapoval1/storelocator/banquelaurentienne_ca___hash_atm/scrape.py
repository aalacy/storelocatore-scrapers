...
def fetch_data():
    ...
    for d in div:
        ...
        abm_in_branch = d.xpath(".//td[4]/div[2]/text()")
        abm_in_branch = list(filter(None, [a.strip() for a in abm_in_branch]))
        abm_in_branch = " ".join(abm_in_branch).replace("\r\n\t\t\t\t\t\t/", "")

        if (
            location_name.find("ABM outside branch") == -1
            and "ABM" not in abm_in_branch
        ):
            continue
