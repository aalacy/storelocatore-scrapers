...
def fetch_data(sgw: SgWriter):
    ...
    api_url = "https://www.jysk.lv/veikali"
    headers = {
    ...
    for j in js["items"]:
        ...
        page_url = "https://www.jysk.lv/veikali"
        ...
        phone = "".join(j.get("phone")).replace("Klientu serviss", "").strip()
        desc = "".join(j.get("description"))
        a = html.fromstring(desc)
        hours_of_operation = (
            " ".join(a.xpath("//*//text()")).replace("\n", "").replace("\r", "").strip()
        )
        if hours_of_operation.find("Darba laiks svētku dienā") != -1:
            hours_of_operation = hours_of_operation.split("Darba laiks svētku dienā")[
                0
            ].strip()
