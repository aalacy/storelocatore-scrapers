...
def fetch_data(sgw: SgWriter):
    ...
    locator_domain = "https://www.tacobell.pt/"
    api_url = "https://www.tacobell.pt/restaurantes/"
    session = SgRequests()
    headers = {
    ...
    for d in div:
        ...
        if street_address.find("1590-392 Avenida Lusiada") != -1:
            street_address = "C.C. Colombo Loja 2.022"
            postal = "1590-392"
        if postal == "1.047 4450-337":
            state = "<MISSING>"
            postal = "4450-337"
            street_address = "C.C. MarShopping Matosinhos, Loja 1.047"
        ...
        r = session.get(page_url, headers=headers)
        tree = html.fromstring(r.text)
        text = "".join(tree.xpath('//div[@am-col="md-6"]/a/@href'))
        try:
            if text.find("ll=") != -1:
