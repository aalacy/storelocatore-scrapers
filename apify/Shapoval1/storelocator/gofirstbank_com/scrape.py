...
def fetch_data(sgw: SgWriter):
    ...
    api_url = "https://www.gofirstbank.com/_/api/branches/44.7532651/-108.7565502/500"
    session = SgRequests()
    headers = {
    ...
    for j in js["branches"]:
        ...
        if hours_of_operation.find("Real Estate Hours") != -1:
            hours_of_operation = hours_of_operation.split("Real Estate Hours")[
                1
            ].strip()
        hours_of_operation = (
            hours_of_operation.replace("*No", "").replace("No", "").strip()
        )
