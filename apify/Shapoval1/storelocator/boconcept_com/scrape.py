...
def fetch_data(sgw: SgWriter):
    ...
    api_url = "https://www.boconcept.com/on/demandware.store/Sites-US-Site/en_US/CountryGateway-ModalContent?format=ajax"
    session = SgRequests()
    headers = {
    ...
    div = tree.xpath('//select[@id="country"]/option')
    for d in div:
        country = "".join(d.xpath(".//@value")).lower()
        session = SgRequests()
        r = session.get(
            f"https://www.boconcept.com/on/demandware.store/Sites-US-Site/en_US/Storelocator-GetStores?country={country}",
            headers=headers,
        )
        js = r.json()["stores"]
        for j in js:
