...
def fetch_data(sgw: SgWriter):
    ...
    div = tree.xpath(
        '//section[@class="zn_section eluid8c9d7270     section-sidemargins    section--no "]//div[./p/a]'
    )
    ...
    for d in div:
        ...
        page_url = (
            "".join(d.xpath('.//a[contains(@href, "tagglogistics")]/@href'))
            or "<MISSING>"
        )
        if page_url.count("https") == 2:
            page_url = (
                "".join(d.xpath('./p/a[contains(@href, "tagglogistics")][1]/@href'))
                or "<MISSING>"
            )
        if page_url == "<MISSING>":
            page_url = (
                "".join(d.xpath('.//a[contains(@href, "order-fulfillment")]/@href'))
                .replace("/order", "order")
                .replace("..", "")
                or "<MISSING>"
            )
        ...
        if page_url == "<MISSING>":
            page_url = (
                "".join(d.xpath(".//p/span[1]/a/@href")).replace("..", "")
                or "<MISSING>"
            )
        if page_url.find("https") == -1 and page_url != "<MISSING>":
            page_url = f"https://www.lesaint.com{page_url}"
