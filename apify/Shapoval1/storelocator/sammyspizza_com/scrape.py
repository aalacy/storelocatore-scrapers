...
def fetch_data():
    ...
    div = ''.join(tree.xpath('//script[contains(text(), "address_details_array.push")]/text()'))
    ...
    for d in divs:
        ...
        location_name = ''.join(tree.xpath('//h2[@class="diy-h3 vb-text-color vb-heading-size vb-text-transform color-3"]/text()'))

        ad = tree.xpath('//h3[contains(text(), "Address")]/following-sibling::p//text()')
        ...
        msslug = city + ' ' + state
        latitude = ll.split(msslug)[1].split(']);')[0].split('", "')[1].split('",')[0].strip()
        longitude = '-' + ll.split(msslug)[1].split('"-')[1].split('", "')[0]
        hours_of_operation = tree.xpath('//h2[contains(text(), "Hours of Operation")]/following-sibling::ul/li//text()')
