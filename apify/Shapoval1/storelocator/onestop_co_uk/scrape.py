...
def get_data(_zip):
    ...
    session = SgRequests()
    r = session.post('https://www.onestop.co.uk/wp-admin/admin-ajax.php', headers=headers, data=data)
    js = r.json()
    if 'locations' in js['message']:
        for loc in js['message']['locations']:
            ...
            country_code = "GB"
            state = '<MISSING>'
            street_address = loc.get('location').get('address').get('details').get('lines')[0]
            if len(loc.get('location').get('address').get('details').get('lines')) == 2:
                street_address = street_address + ' ' + loc.get('location').get('address').get('details').get('lines')[1]
            city = loc.get('location').get('address').get('details').get('postTown') or '<MISSING>'

            postal = loc.get('location').get('address').get('details').get('postcode') or '<MISSING>'
            phone = loc.get('location').get('contact').get('phoneNumbers').get('main') or '<MISSING>'
            latitude = loc.get('location').get('coordinates').get('latitude') or '<MISSING>'
            longitude = loc.get('location').get('coordinates').get('longitude') or '<MISSING>'
            ...
            hours_of_operation = monday_hrs + ' ' + tuesday_hrs + ' ' + wednesday_hrs + ' ' + thursday_hrs + ' ' + friday_hrs + ' ' + saturday_hrs + ' ' + sunday_hrs or '<MISSING>'
