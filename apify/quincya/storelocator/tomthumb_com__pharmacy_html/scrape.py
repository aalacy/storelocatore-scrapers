...
logger = SgLogSetup().get_logger("tomthumb.com")


def fetch_data(sgw: SgWriter):
    base_link = "https://local.pharmacy.tomthumb.com/index.html"
    ...
    final_links = []
    locator_domain = "tomthumb.com"

    state_links = []
    states = base.find_all(class_="Directory-listLink")
    for state in states:
        state_link = "https://local.pharmacy.tomthumb.com/" + state["href"]
    ...
    for state_link in state_links:
        ...
        for city in cities:
            city_link = "https://local.pharmacy.tomthumb.com/" + city["href"].replace(
                "../", ""
            )
            ...
            if count == "1":
                ...
                for final_item in final_items:
                    final_link = (
                        "https://local.pharmacy.tomthumb.com/" + final_item["href"]
                    ).replace("../", "")
    ...
    for final_link in final_links:
        ...
        base = BeautifulSoup(req.text, "lxml")
        location_name = base.find(class_="ContentBanner-h1").text.strip()
        ...
        try:
            phone = (
                base.find(string="Pharmacy Phone:")
                .find_next()
                .find(id="phone-main")
                .text.strip()
            )
        ...
        try:
            hours_of_operation = (
                " ".join(
                    list(
                        base.find(string="Pharmacy Hours")
                        .find_next("table")
                        .stripped_strings
                    )
                )
