...
def fetch_data():
	...
	items = base.find_all(class_="nectar-button small regular extra-color-1 regular-button")
	...
	for item in items:
		...
		street_address = base.find_all(class_="wpb_wrapper")[1].find_all("p")[-1].text.split("\n")[0].split(",")[0].replace("Portland","").strip()
		city = base.h5.text.split(",")[0].strip()
		state = base.h5.text.split(",")[1].strip()
		zip_code = base.find_all(class_="wpb_wrapper")[1].find_all("p")[-1].text.split("\n")[0][-5:].strip()
