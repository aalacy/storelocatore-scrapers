...
def fetch_data():
    ...
    WebDriverWait(driver, 30).until(
        ec.presence_of_element_located((By.CLASS_NAME, "Teaser-title"))
    )
    time.sleep(2)
    ...
    while True:
        ...
        try:
            next_page = driver.find_element_by_css_selector(
                ".Results-seeMore.Button--hollow.js-locator-seeMore"
            )
    ...
    for item in items:
        ...
        try:
            street_address = (
                item.find(class_="c-address-street-1").text.strip()
                + " "
        ...
        try:
            ...
            link = "<MISSING>"
            if location_name in [
                "Morgan Stanley Pasadena Branch",
                "Morgan Stanley Austin Branch",
                "Morgan Stanley Wichita Branch",
                "Morgan Stanley Short Hills Branch",
            ]:
                continue
