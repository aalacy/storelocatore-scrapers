	...
	for city_link in city_links:
		...
		for item in items:
			...
			if "branch" in location_type.lower():
				...
				street_address = base.find('meta', attrs={'property': "og:street-address"})['content']
				...
				try:
					hours_of_operation = " ".join(list(base.find(id="primary-hours").find(class_="hours").stripped_strings)).replace("Holiday Hours","").replace("  "," ")
				except:
					hours_of_operation = " ".join(list(base.find(class_="hours").find(class_="hours").stripped_strings)).replace("Holiday Hours","").replace("  "," ")

				if hours_of_operation == "Mon Closed Tue Closed Wed Closed Thu Closed Fri Closed Sat Closed Sun Closed":
