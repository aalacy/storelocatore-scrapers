...
def parse_detail(store, link):
    ...
    phone = get_value(store.xpath('.//span[@itemprop="telephone"]//text()'))
    street = get_value(store.xpath('.//span[@itemprop="streetAddress"]//text()'))
    if (street + phone) in found_street:
        return "skip"
    found_street.append(street + phone)
    output.append(phone)  # phone
    if "branch" in name.lower():
        location_type = "ATM, Branch"
    elif "atm" in name.lower():
        location_type = "ATM"
    else:
    ...
    if "ATM" not in str(store.xpath(".//text()")).upper():
        return "skip"
    raw_hours = store.xpath('.//div[@class="LocationInfo-atmHours"]//text()')
    if "ATM HOURS" not in str(raw_hours).upper():
        raw_hours = store.xpath('.//div[@class="LocationInfo-hours"]//text()')
    if "ATM HOURS" not in str(raw_hours).upper():
        raw_hours = "<MISSING>"
