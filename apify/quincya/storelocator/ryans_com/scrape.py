...
logger = SgLogSetup().get_logger('ryansrestaurants_com')
...
def fetch_data():
	...
	data = []
	locator_domain = "ryansrestaurants.com"

	for i in range(52):

		base_link = "http://www.ryansrestaurants.com/index.php?usahtml5map_get_state_info=%s&map_id=0" %i
		...
		for item in items:
			...
			try:
				location_name = item.div.div.text.encode("ascii", "replace").decode().replace("?","'").strip()
			except:
				pass

			if "ryan" not in location_name.lower():
				continue
