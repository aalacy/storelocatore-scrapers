...
def fetch_data():
    ...
    for item in items:
        if ',title:"' in item:
            location_name = item.split('title:"')[1].split('",click')[0].replace("amp;","").strip()
            country_code = "US"
            ...
            try:
                link = item.split('href="')[1].split('"')[0].strip()
                if link[-2:] == "//":
                    link = link[:-1]
            except:
                link = "<MISSING>"
            ...
            if link != "<MISSING>" and "www.medstarhealth.org/mhs" in link:
                ...
                try:
                    ...
                    if len(raw_address) < 2:
                        raw_address = list(soup.find_all(class_="well")[1].h4.stripped_strings)
                        raw_address = item.split('<br>')[1:-1]
                        phone = raw_address[-1].strip()
                    else:
                        phone = soup.find(class_="well").h2.text.split(":")[1].strip()
            ...
            if not done:
                raw_address = ",".join(item.replace("MD<br>","MD ").split('<br />')[1:-1]).replace(",<b","").replace("705t","705").split(",")
                ...
                if not raw_address[0]:
                    ...
                    if len(raw_address) == 1:
                        raw_address = ",".join(item.replace("MD<br>","MD ").split('</a><br>')[1].split('</div>')[0][:-4].split("<br>")).replace(",<b","").split(",")
            ...
            if len(zip_code.strip()) != 5:
                try:
                    zip_code = re.findall(" [\d]{5}<", str(item))[0].replace("<","").strip()
                except:
                    zip_code = "<MISSING>"
