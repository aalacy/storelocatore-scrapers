...
def fetch_data():
	...
	for item in items:
		...
		raw_address = base.title.text.replace("S. ","S ").replace("N. ","N ").replace("W. ","W ").replace("Miami Fl","Miami, Fl").replace("Way Coral","Way. Coral")\
		.replace("St Miami","St. Miami").replace("Street Miami","Street. Miami").replace("FL3","FL 3").split(",")
		...
		try:
			geo = re.findall(r'[0-9]{2}\.[0-9]+,-[0-9]{2,3}\.[0-9]+',map_link)[-1].split(",")
			latitude = geo[0]
			longitude = geo[1]
		except:
			try:
