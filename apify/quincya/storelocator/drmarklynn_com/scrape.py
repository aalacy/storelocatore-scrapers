...
def fetch_data():
    ...
    for num in range(1,4):
        base_link = "https://drmarklynn.com/wp-json/wp/v2/doctor?page=%s&per_page=100&_embed" %num

        session = SgRequests()
        req = session.get(base_link, headers = HEADERS)

        base = BeautifulSoup(req.text,"lxml")
        stores = stores = json.loads(base.text.strip())

        try:
            if stores["data"]["status"] == 400:
                break
        except:
            pass

        for i in stores:
            store = i["acf"]["locations_doctors"][0]["acf"]
