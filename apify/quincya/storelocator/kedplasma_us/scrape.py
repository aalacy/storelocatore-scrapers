...
def fetch_data():
	...
	for item in items:
		...
		hours_of_operation = " ".join((list(base.find(class_="plasma-center-contact-us").stripped_strings)[:-1])).replace("\xa0"," ").replace("Opening hours:","").split("KEDPLASMA")[0].split("Se hab")[0].strip()
		...
		for script in all_scripts:
			if "myLatlng" in str(script):
				script = str(script)
				geo = re.findall(r'[0-9]{2}\.[0-9]+,-[0-9]{2,3}\.[0-9]+',script)[0].split(",")
				latitude = geo[0]
				longitude = geo[1]
				break
