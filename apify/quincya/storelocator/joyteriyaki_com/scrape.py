...
def fetch_data():
	...
	items = base.find(id="c9tkinlineContent").find_all(class_="font_7")
	for item in items:
	...
	for i in range(len(names)):
		...
		location_name = names[i].text.encode("ascii", "replace").decode().replace("?"," ").strip()

		raw_address = locs[i].text.replace("\xa0"," ").strip()

		street_address = " ".join(raw_address[:raw_address.rfind(",")].split()[:-1]).replace("104,","104").strip()
