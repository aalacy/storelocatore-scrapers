...
def fetch_data():
    ...
    r = r.text.split('"galleryImages":[],"locations":[{',1)[1].split('"socialHandles":[]}]',1)[0]
    r = '[{'+r + '"socialHandles":[]}]'
    ...
    for loc in loclist:
        ...
        if link.find('bend-tap-room') > -1:
            link = link.split('-')[0]
        phone = loc['phone'][0:3]+'-'+loc['phone'][3:6]+'-'+loc['phone'][6:len(loc['phone'])]
        hours = ''
        hourd= []
        hourd.append('none')
        print(title)
        try:
            for hr in hourlist:
                ...
                if dt in hourd:
                    ...
                    if day > 12 :
                        day = day - 12
                    hours = hours + hr.split('-')[0]+ ' am '+ ' - ' + str(day) + ":00 PM" 
                    hours = hours + ' '
