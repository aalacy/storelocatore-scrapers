...
DOMAIN = "https://www.barresoul.com"
...
def fetch_data():
    if True:
        ...
        loclist = soup.find("div", {"class": "sqs-gallery"}).findAll(
            "div", {"class": "slide"}
        )

        for loc in loclist:
            page_url = loc.find("a")["href"]
            if "virtual" in page_url:
                continue
            log.info(page_url)
            ...
            temp = temp.find("div", {"class": "sqs-block-html"})
            try:
                address = (
                    temp.find("h3")
                ...
                address = (
                    temp.find("p").get_text(separator="|", strip=True).replace("|", " ")
                )
            phone = (
