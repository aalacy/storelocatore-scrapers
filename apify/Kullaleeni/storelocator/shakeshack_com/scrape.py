...
def fetch_data():
    ...
    for div in divlist:
        ...
        # input()
        for dt in det:
            temp = dt.replace('.', '').replace('-', '').replace(')', '').replace('(', '').replace(' ',
                                                                                                  '').lstrip().rstrip()
            temp = temp.replace('–', '')
        ...
        while i < len(address):
            temp = address[i]
            if temp[1].find("Address") != -1 or temp[1].find("Street") != -1 or temp[1].find('Occupancy') != -1 or temp[
                1].find("Recipient") != -1 or temp[1].find("BuildingName") != -1 or temp[1].find("USPSBoxType") != -1 or \
                            temp[1].find("USPSBoxID") != -1:
