...
def fetch_data():
    out = []
    locator_domain = "https://cafevanhoutte.com/"
    api_url = "https://cafevanhoutte.com/wp-content/plugins/superstorefinder-wp/ssf-wp-xml.php?wpml_lang=en"
    page_url = "https://cafevanhoutte.com/find-a-cafe/"
    ...
    for i in items:
        ...
        if "« Temporary Closed »" in line:
            hours_of_operation = "Temporary Closed"
            line = line.replace("« Temporary Closed »", "").strip()
        else:
        ...
        if city == "Ottawa" and state == "<MISSING>":
