...
def fetch_data():
    ...
    for u in urls:
        ...
        locator_domain = url
        location_name = (
            "".join(tree.xpath("//h1[@class='heading']/text()"))
            .replace("&#xe9;", "é")
            .strip()
        )
        street_address = "".join(
        ...
        city = "".join(
            tree.xpath("//meta[@property='business:contact_data:locality']/@content")
        ).replace("&#Xe9;", "é")
        state = "".join(tree.xpath("//a[contains(@href, 'maps.apple')]/@href")).split(
            ","
        )[-2]
        postal = "".join(
            tree.xpath("//meta[@property='business:contact_data:postal_code']/@content")
        )
        country_code = "".join(
            tree.xpath(
