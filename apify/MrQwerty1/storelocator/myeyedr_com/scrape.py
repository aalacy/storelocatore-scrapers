...
def fetch_data():
    out = []
    url = 'https://locations.myeyedr.com/'
    ...
    for i in range(0, 100000, 10):
        r = session.get(f'https://locations.myeyedr.com/search?q=&offset={i}&c=us', headers=headers)
        ...
        for jj in js:
            ...
            location_name = f"{j.get('name')} in {city}"
            state = a.get('region') or '<MISSING>'
            postal = a.get('postalCode') or '<MISSING>'
            country_code = a.get('countryCode') or '<MISSING>'
            store_number = '<MISSING>'
            page_url = j.get('websiteUrl') if j.get('websiteUrl') else j.get('c_pagesURL') or '<MISSING>'
