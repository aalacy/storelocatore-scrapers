...
def fetch_data(sgw: SgWriter):
    for i in range(0, 100000, 50):
        r = session.get(
            f"https://location.steers.co.za/search?per=50&offset={i}",
            headers=headers,
        )
        ...
        for jj in js:
            j = jj.get("profile")
            a = j.get("address")
            page_url = j.get("c_pagesURL") or ""
            street_address = a.get("line1") or ""
            city = a.get("city") or ""
            location_name = a.get("extraDescription") or ""
...
if __name__ == "__main__":
    locator_domain = "https://steers.co.za/"
    headers = {
