...
def get_ids():
    ids = []
    session = SgRequests()
    r = session.get("https://www.crunch.com/crunch_core/clubs")
    js = r.json()
    for j in js:
        ids.append(j["id"])
...
def get_data(_id):
    ...
    api_url = f"https://www.crunch.com/crunch_core/clubs/{_id}"

    session = SgRequests()
    r = session.get(api_url)
    j = r.json()

    page_url = f'https://www.crunch.com/locations/{j.get("slug")}'
