...
def get_urls():
    ...
    return tree.xpath(
        "//loc[contains(text(), 'eyemartexpress.com/get-glasses/')]/text()"
    )
...
def get_data(url):
    ...
    street_address = (
        "".join(tree.xpath("//p[@class='mt-1 m-0']/text()")).strip() or "<MISSING>"
    )
    a = "".join(tree.xpath("//div[@itemprop='address']/p[@class='m-0']/text()")).strip()
    ...
    phone = (
        "".join(
            tree.xpath("//p[@class='my-h-mobile']/a[contains(@href, 'tel:')]/text()")
        )
        or "<MISSING>"
    )
    g = (
        "".join(tree.xpath("//a[contains(@href, '/maps/search/')]/@href"))
        .split("&query=")[1]
        .split("&")[0]
    )
    latitude = g.split(",")[0]
