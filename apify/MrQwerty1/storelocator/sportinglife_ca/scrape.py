...
def get_ids():
    session = SgRequests()
    r = session.get("https://stores.sportinglife.ca/en")
...
def get_urls():
    ...
    for b in batches:
        url = f'https://sls-api-service.sweetiq-sls-production-west.sweetiq.com/bu0CZYVJ3RYzVPxbBx5en7FF68xew9/locations-details?locale=en_CA&ids={",".join(b)}&cname=stores.sportinglife.ca'
        urls.append(url)
...
def get_data(url):
    rows = []
    locator_domain = "https://sportinglife.ca/"
