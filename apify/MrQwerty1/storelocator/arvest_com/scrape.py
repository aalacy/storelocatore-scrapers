...
def fetch_data():
    out = []
    locator_domain = "https://arvest.com/"

    session = SgRequests()

    for i in range(1, 10000):
        api_url = f"https://api.momentfeed.com/v1/analytics/api/llp.json?auth_token=RCKBBWJSPPOONTUT&pageSize=100&page={i}"
        r = session.get(api_url)
        js = r.json()

        for j in js:
            page_url = "https://locations.arvest.com/" + j.get("llp_url") or "<MISSING>"
