...
def fetch_data():
    ...
    for i in range(0, 100000, 10):
        r = session.get(f'https://stores.express.com/search?q=&offset={i}&c=us&storeType=4067209', headers=headers)
        ...
        for jj in js:
            ...
            location_name = f"Express Factory Outlet {j.get('c_storeName')}"
            state = a.get('region') or '<MISSING>'
            postal = a.get('postalCode') or '<MISSING>'
            country_code = a.get('countryCode') or '<MISSING>'
            store_number = '<MISSING>'
            q = f'{city},+{state},+{country_code}'
            page_url = j.get('c_pagesUrl') or f'https://stores.express.com/search?storeType=4067209&q={q}'
