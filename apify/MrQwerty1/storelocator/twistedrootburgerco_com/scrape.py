...
def get_urls():
    ...
    return tree.xpath("//h2[@class='contentTitle']/a/@href")
...
def get_data(url):
    ...
    location_name = "".join(
        tree.xpath("//div[@class='blockText']/h2[@class='contentTitle']/text()")
    ).strip()
    line = tree.xpath("//h2[text()='Location']/following-sibling::div//text()")
    line = list(filter(None, [l.strip() for l in line]))

    if len(line) == 1:
        adr = line[0].replace("&142", "")
    else:
        adr = ", ".join(line[:-1])
    ...
    phone = line[-1]
    if phone.find("CALL") != -1:
        phone = phone.split("CALL")[-1].strip()
    if phone.find("@") != -1 or len(phone) > 15:
        phone = "<MISSING>"
