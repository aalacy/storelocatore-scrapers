...
def generate_links():
    urls = []
    session = SgRequests()
    r = session.get("https://stores.dorothyperkins.com/index.html")
    tree = html.fromstring(r.text)
    states = tree.xpath("//a[@class='Directory-listLink']/@href")
    for s in states:
        if s.find("/") != -1:
            s = s.split("/")[0]
        r = session.get(f"https://stores.dorothyperkins.com/{s}.json")
...
def get_urls(states):
    for state in states.values():
        ...
        if children is None:
            yield f"https://stores.dorothyperkins.com/{state['url']}.json"
        else:
...
def get_data(url):
    ...
    locator_domain = "https://www.dorothyperkins.com/"
