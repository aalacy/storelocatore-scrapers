...
def fetch_data():
    ...
    for j in js:
        ...
        li = tree.xpath(f"//li[@class='grid grid--locations grid--one-col-mobile locations-list__list-item' and .//a[contains(text(), '-{_id}')]]")[0]
        city = ''.join(li.xpath("./text()")).replace('.', '').strip()
        line = ''.join(li.xpath(".//li[@class='locations-list__phone-number']/preceding-sibling::li[1]/text()")).replace('.', '').strip()
