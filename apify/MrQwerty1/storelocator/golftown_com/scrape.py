...
def get_ids():
    session = SgRequests()
    r = session.get("https://stores.golftown.com/")
...
def get_urls():
    ...
    for b in batches:
        url = f'https://sls-api-service.sweetiq-sls-production-east.sweetiq.com/N3336IgSSWOJPw8WlNino5JTtyHjAw/locations-details?locale=en_CA&ids={",".join(b)}&cname=stores.golftown.com'
        urls.append(url)
...
def get_data(url):
    rows = []
    locator_domain = "https://golftown.com/"
    ...
    for j in js:
        ...
        page_url = f'https://stores.golftown.com/{j.get("slug")}'
        street_address = (
