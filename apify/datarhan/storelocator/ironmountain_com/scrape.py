    ...
    for location_url in all_locations_urls:
        ...
        for point_url in all_points:
            ...
            for point_num, point_location in enumerate(all_point_locations):
                ...
                location_name = point_location.xpath('.//span[@class="location-name capitalize bold text-blue"]/text()')[0]
                location_name = location_name.strip() if location_name else '<MISSING>'
                street_address = point_location.xpath('.//div[@data-hide-address]/text()')[0]
                street_address = street_address if street_address else '<MISSING>'
                raw_adr_data = point_location.xpath('.//div[@class="country-name"]/preceding-sibling::div[1]/text()')[0]
                ...
                country_code = point_location.xpath('.//div[@class="country-name"]/@data-show-uk')[0]
                country_code = country_code if country_code else '<MISSING>'
                store_number = point_location.xpath('@data-fid')[0]
                phone = point_location.xpath('.//div[@class="phone italic font-sm mb-10"]/@data-hide-empty')[0]
                phone = phone if phone else '<MISSING>'
                location_type = '<MISSING>'
                
                geo_data = re.findall('RLS.defaultData =(.+);', etree.tostring(point_dom.xpath('//div[@id="gmap"]')[0]).decode("utf-8").replace('\n', ''))[0]
                ...
                if store_number not in scraped_items:
                    scraped_items.add(store_number)
                    items.append(item)
