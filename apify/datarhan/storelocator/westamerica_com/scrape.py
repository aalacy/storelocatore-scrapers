...
def fetch_data():
    ...
    for city_html in all_locations:
        ...
        for location_name in sub_locs_names:
            print(location_name)
            all_elements = city_html.xpath('//h2[contains(text(), "%s")]/following-sibling::*' % location_name)
            all_sub_locs = []
            ...
            for i, sub_address in enumerate(all_sub_addresses):
                ...
                if "ATM" in phone:
                    ...
                    phone = "<MISSING>"
                try:
                    latitude = all_geo[i][-1].split('sll=')[-1].split('&')[0].split(',')[0]
                    longitude = all_geo[i][-1].split('sll=')[-1].split('&')[0].split(',')[-1]
                except:
                    latitude = "<MISSING>"
                    longitude = "<MISSING>"
                try:
