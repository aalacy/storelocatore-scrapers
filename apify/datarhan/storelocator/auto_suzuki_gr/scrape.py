...
def fetch_data():
    ...
    for poi_html in all_locations:
        ...
        phone = poi_html.xpath('.//a[contains(@href, "tel")]/text()')[0].split(',')[0].split('/')[0].strip()
        ...
        if not poi_html.xpath(f'.//img[contains(@src, "{service}")]'):
            continue
        hoo = poi_html.xpath('.//h6[contains(text(), "Ώρες λειτουργίας έκθεσης")]/following-sibling::p[1]/strong/text()')
        hoo = ' '.join(hoo)
