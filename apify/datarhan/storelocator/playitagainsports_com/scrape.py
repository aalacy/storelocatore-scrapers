...
def fetch_data():
    ...
    with SgFirefox() as driver:
        headers = SgSeleniumDelay.get_default_headers_for(driver, 'https://www.playitagainsports.com/locations')
        for elem in headers:
    ...
    if total_pages > 1:
        ...
        while total_pages != page_num:
            for page in range(2, total_pages + 1):
                all_locations_url = 'https://api.ordercloud.io/v1/suppliers?pageSize=100&page={}&Active=true&sortBy=Name'.format(str(page))
                response = session.get(all_locations_url, headers=hdr)
                data = json.loads(response.text)
                for location_data in data['Items']:
                    all_items.append(location_data)
    ...
    for location_data in all_items:
        ...
        store_url = 'https://www.playitagainsports.com/locations/{}'.format(location_data['xp']['slug'])
