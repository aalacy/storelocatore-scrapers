...
def fetch_data():
    ...
    json_text = (
        stores_req.text.split(
            "ReactDOM.render(React.createElement(Components.LocationPage,"
        )[1]
        .strip()
        .split("),")[0]
        .strip()
    )
    stores = json.loads(json_text)["offices"]
    ...
    for store in stores:
        ...
        if country_code == "USA" or country_code == "CAN":
            curr_list = [
                locator_domain,
                page_url,
                location_name,
