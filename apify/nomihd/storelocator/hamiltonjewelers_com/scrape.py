...
website = "hamiltonjewelers.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "www.hamiltonjewelers.com",
...
def fetch_data():
    # Your scraper here
    base = "https://www.hamiltonjewelers.com"
    search_url = "https://www.hamiltonjewelers.com/pages/Locations.html"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = search_sel.xpath(
        '//div[contains(@class,"foot-box") and ./h4 and ./p]//a'
    )
            ...
            filter(
                ...
                [
                    ...
                    for x in store_sel.xpath(
                        '//div[contains(./div/h2/text(),"Address")]/following-sibling::div//text()'
                    )
                ],
            )
        )
        ...
        phone = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store_sel.xpath(
                        '//div[contains(./div/h2/text(),"Phone")]/following-sibling::div//text()'
                    )
                ],
            )
        )
        ...
        hours = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store_sel.xpath(
                        '//div[contains(./div/h2/text(),"hours")]/following-sibling::div//text()'
                    )
                ],
            )
        )
