...
def fetch_data():
    ...
    search_url = "https://opticaloutlets.com/locations/"
    stores_req = session.get(search_url, headers=headers)
    stores_sel = lxml.html.fromstring(stores_req.text)
    stores = stores_sel.xpath('//div[@class="location_content"]')
    for store in stores:
        page_url = "https://opticaloutlets.com" + "".join(
            store.xpath('div/a[contains(text(),"More Info")]/@href')
        )
        phone = "".join(store.xpath('div/a[contains(@href,"tel:")]/text()')).strip()
        ...
        address = store_sel.xpath(
            '//div[@class="address_container"]/div/div[@class="address"]/text()'
        )[1:]
        ...
        hours = store_sel.xpath(
            '//div[@class="address_container"]/div[@data-micro-template="office_hours"]/div'
        )
        hours_list = []
        for hour in hours:
