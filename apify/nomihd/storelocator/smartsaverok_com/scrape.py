...
def fetch_data():
    ...
    store_list = list(set(search_sel.xpath('//div[contains(@class,"panel-body")]')))
    ...
    for store in store_list:
        ...
        full_address = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store.xpath('.//div[@class="col-md-5"]/p[last()]//text()')
                ],
            )
        )
        ...
        phone = "".join(
            list(
                filter(
                    ...
                    [
                        x.strip()
                        for x in store.xpath('.//*[./i[@class="fa fa-phone"]]/text()')
                    ],
                )
            )
        ).strip()
            ...
            filter(
                ...
                [
                    ...
                    for x in store.xpath(
                        './/p[./b[contains(.//text(),"Store Hours")]]/text()'
                    )
                ],
            )
        )
        ...
        latitude = lat_lng_href.split("place/")[1].split(",")[0].strip()
        longitude = lat_lng_href.split("place/")[1].split(",")[1].strip()
