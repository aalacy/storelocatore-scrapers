...
website = "harterhouse.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "harterhouse.com",
...
def fetch_data():
    # Your scraper here
    base = "https://harterhouse.com"
    search_url = "http://harterhouse.com/index.php"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = list(search_sel.xpath('//div[contains(@class,"locations")]'))

    for store in store_list:

        page_url = base + "".join(store.xpath("./div/a/@href"))

        if base + "/" not in page_url:
            page_url = base + "/" + "".join(store.xpath("./div/a/@href"))
        ...
        store_info = list(
            filter(
                str,
                [x.strip() for x in store_sel.xpath('//*[@class="store-add"]//text()')],
            )
        )[:-1]
        ...
        if not hours_of_operation:
            hours_of_operation = store_info[phn_idx].split("Hours:")[1].strip()
