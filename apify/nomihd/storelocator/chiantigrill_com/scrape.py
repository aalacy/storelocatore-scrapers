...
website = "chiantigrill.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "www.chiantigrill.com",
...
def fetch_data():
    # Your scraper here
    search_url = "https://www.chiantigrill.com/contact"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = search_sel.xpath(
        '//div[.//a[contains(@href,"maps")] and @data-testid="richTextElement"]'
    )
    ...
    for store in store_list:
        ...
        store_info = list(
            filter(
                str,
                [x.strip() for x in store.xpath(".//a//text()")],
            )
        )
        ...
        location_name = f"CHIANTI GRILL - {city}"
            ...
            filter(
                ...
                [
                    ...
                    for x in search_sel.xpath(
                        '//span[contains(@style,"letter-spacing:0.03em") and text()]/text()'
                    )
                ],
            )
        )[0]
        ...
        hours_of_operation = (
            hours.split("Please be aware")[0].replace("Currently we are ", "").strip()
        )

        map_link = store.xpath('.//a[contains(@href,"maps")]/@href')[0]
