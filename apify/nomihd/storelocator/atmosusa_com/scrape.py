...
website = "atmosusa.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "www.atmosusa.com",
...
def fetch_data():
    # Your scraper here
    search_url = "https://www.atmosusa.com/pages/locations"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = list(search_sel.xpath('//*[@class="Section "]'))
    ...
    for store in store_list:
        ...
        store_info = list(
            filter(
                str,
                [x.strip() for x in store.xpath(".//p[not(strong) and text()]/text()")],
            )
        )
        ...
        location_name = f"AtmosUSA_{city}"
        ...
        hours_of_operation = (
            "; ".join(hours).strip().replace(";", ":").replace("pm:", "pm; ").strip()
        )
