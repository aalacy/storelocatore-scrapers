...
def fetch_data():
    ...
    stores_sel = etree.fromstring(xml_text)
    ...
    for store in stores:
        ...
        for child in store:
            ...
            if child.tag == "label":
                store_number = child.text.replace("Store", "").strip()
            if child.tag == "infowindow":
                html_data = lxml.html.fromstring(
                    child.text.replace("&lt;", "<")
                    .replace("&quot;", '"')
                    .replace("&gt;", ">")
                    .strip()
                ...
                page_url = "".join(
                    html_data.xpath('//a[@class="detail-btn"]/@href')
                ).strip()
        ...
        location_name = "".join(
            store_sel.xpath('//div[@class="single-storenumber"]/h2/text()')
        ).strip()
        address = store_sel.xpath('//div[@class="single-storeaddress"]/text()')
        add_list = []
        for add in address:
        ...
        phone = (
            "".join(store_sel.xpath('//a[contains(@href,"tel:")]/@href'))
            .strip()
            .replace("tel:", "")
            .strip()
        )
