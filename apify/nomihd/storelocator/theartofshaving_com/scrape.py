...
website = "theartofshaving.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "theartofshaving.com",
...
def fetch_data():
    # Your scraper here
    base = "https://theartofshaving.com"
    search_url = "https://theartofshaving.com/stores-find.html"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = search_sel.xpath('//div[@class="store-shop"]')

    for store in store_list:

        page_url = base + store.xpath("./a/@href")[0].strip()
        ...
        store_info = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store_sel.xpath('//div[@class="address1"]//text()')
                ],
            )
        )
        ...
        hours = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store_sel.xpath('//div[@class="store-hours"]//text()')
                ],
            )
        )
