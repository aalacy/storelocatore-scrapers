...
def fetch_data():
    # Your scraper here
    search_url = "https://barrioqueen.com/location/"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = search_sel.xpath(
        '//div[contains(@class,"fusion-text ") and .//p[not(@class) and not(text())]]'
    )
    ...
    for store in store_list:
        ...
        store_info = list(
            filter(
                str,
                [x.strip() for x in store.xpath(".//p[not(strong)]//text()")],
            )
        )
        ...
        if re.search("^[0-9-.() ]{1,17}$", store_info[phn_idx]):
            address_raw = " ".join(store_info[1:phn_idx])
            phone = store_info[phn_idx].strip()
        else:
            address_raw = " ".join(store_info[1:])
            phone = "<MISSING>"
        ...
        location_name = f"Barrioqueen_{store_info[0]}"
