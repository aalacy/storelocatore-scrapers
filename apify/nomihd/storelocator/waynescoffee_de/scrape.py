...
def fetch_data():
    # Your scraper here
    search_url = "https://www.waynescoffee.de/standorte/"
    with SgRequests() as session:
        ...
        for store in stores:
            ...
            state = "<MISSING>"
            zip = store_sel.xpath(
                '//div[@class="cafe-information-row"][./h2[contains(text(),"Adresse")]]/p[1]/text()'
            )

            if len(zip) > 0:
                zip = zip[-1].rsplit(" ")[-2].strip()
