...
website = "phoenixtheatres.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "www.phoenixtheatres.com",
...
def fetch_data():
    # Your scraper here
    base = "https://www.phoenixtheatres.com"
    search_url = "https://www.phoenixtheatres.com/locations"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = search_sel.xpath('//div[@class="theatre-info"]')
    ...
    for store in store_list:
        ...
        store_info = list(
            filter(
                str,
                [x.strip() for x in store_sel.xpath('//*[@id="address"]//text()')],
            )
        )
        ...
        phone = store_info[-1].strip().upper().replace("PHONE:", "").strip()
