...
def fetch_data():
    ...
    search_url = "https://childrensparadise.com/"
    search_res = session.get(search_url, headers=headers)
    search_sel = lxml.html.fromstring(search_res.text)

    stores_list = search_sel.xpath('//li[contains(.//text(),"Locations")]//li/a/@href')
    ...
    for store in stores_list:
        ...
        address_info = list(
            filter(
                str,
                page_sel.xpath(
                    '//div[contains(@class,"ContactDetails")]//span[not(./a)]//text()'
                ),
            )
        )

        street_address = " ".join(address_info[1:-1]).strip()
        ...
        lat_lng_href = "".join(
            page_sel.xpath('//div[contains(@class,"ContactDetails")]//span[./a]//@href')
        )
