    ...
    with SgRequests(dont_retry_status_codes=([404])) as session:
        ...
        while True:
            ...
            for store in stores:
                ...
                store_number = "".join(store.xpath(".//a[@data-no]/@data-no")).strip()
                phone = "".join(store.xpath("td[4]/text()")).strip()
                location_type = "".join(store.xpath("td[1]/img/@alt")).strip()
            ...
            if len(next_page) > 0:
                if "javascript" in next_page[0]:
                    ...
                    search_url = (
                        "https://www.columbiakorea.co.kr/about/store.asp" + next_page[0]
                    )
            else:
                break
