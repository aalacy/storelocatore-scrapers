...
def fetch_data():
    # Your scraper here
    search_url = "https://milkandhoneyspa.com/locations/"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = list(search_sel.xpath('//div[@class="location-page"]'))

    for store in store_list:

        page_url = "".join(
            store.xpath(
                './/form[contains(@action,"https://milkandhoneyspa.com/locations")]/@action'
            )
        )
        ...
        full_address.append(",".join(store_info[1:]))
