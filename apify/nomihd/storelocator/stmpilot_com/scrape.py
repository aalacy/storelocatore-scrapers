...
domain = "https://locations.stmtires.com/"
...
def get_store_data(store_sel, page_url):
    ...
    location_name = (
        " ".join(store_sel.xpath('//h1[@class="Hero-heading"]//text()'))
        .strip()
        .replace("  ", " ")
        .strip()
    )
    street_address = "".join(
        store_sel.xpath(
            '//div[@class="Address Hero-address"]/address//span[@class="Address-field Address-line1"]/text()'
        )
    ).strip()

    add_2 = "".join(
        store_sel.xpath(
            '//div[@class="Address Hero-address"]/address//span[@class="Address-field Address-line2"]/text()'
        )
    ).strip()
    ...
    city = "".join(
        store_sel.xpath(
            '//div[@class="Address Hero-address"]/address//span[@class="Address-field Address-city"]/text()'
        )
    ).strip()

    state = "".join(
        store_sel.xpath(
            '//div[@class="Address Hero-address"]/address//span[@itemprop="addressRegion"]/text()'
        )
    ).strip()

    zip = "".join(
        store_sel.xpath(
            '//div[@class="Address Hero-address"]/address//span[@class="Address-field Address-postalCode"]/text()'
        )
    ).strip()
...
def fetch_data():
    # Your scraper here
    states_sel = get_selector("https://locations.stmtires.com/")
