...
website = "border-tire.com"
log = sglog.SgLogSetup().get_logger(logger_name=website)
session = SgRequests()
headers = {
    "authority": "www.border-tire.com",
...
def fetch_data():
    # Your scraper here
    search_url = "https://www.border-tire.com/locations"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    sections = search_sel.xpath('//div[@class="section"]')
    for section in sections:
        breakline_count = len(section.xpath(".//br"))
        count = 0
        while count <= breakline_count:
            page_url = search_url
                ...
                filter(
                    ...
                    [
                        ...
                        for x in section.xpath(
                            f".//p[ count(preceding-sibling::br)={count}]//text()"
                        )
                    ],
                )
            )
