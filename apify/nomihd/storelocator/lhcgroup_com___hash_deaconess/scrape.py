...
website = "https://lhcgroup.com/#deaconess"
...
def fetch_data():
    ...
    for store in stores:
        ...
        if len(phone) <= 0:
            phone = "".join(
                store_sel.xpath(
                    '//div[@class="info-item small"][.//use[contains(@href,"#icon_phone")]]/div[1]/text()'
                )
            ).strip()
        ...
        hours_of_operation = "".join(
            store_sel.xpath(
                '//div[@class="info-item small"][.//use[contains(@href,"#icon_hours")]]/div/text()'
            )
        ).strip()
