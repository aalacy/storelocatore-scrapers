...
session = SgRequests()
headers = {
    "authority": "www.signaturebank.bank",
...
def fetch_data():
    # Your scraper here
    search_url = "https://www.signaturebank.bank/contact/#map"
    ...
    for store in store_list:
        ...
        location_name = "".join(store.xpath(".//div[./h3]/h3//text()")).strip()
        phone = list(
            filter(
                str,
        ...
        phone = "".join(phone).split("F")[0].strip("P: ").strip()
        ...
        longitude = store.xpath('.//div[contains(@class,"acf-map")]/div/@data-lng')[
            0
        ].strip()
