...
def fetch_data():
    # Your scraper here
    base = "https://www.poppin.com"
    search_url = "https://www.poppin.com/showrooms/"
    search_res = session.get(search_url, headers=headers)

    search_sel = lxml.html.fromstring(search_res.text)

    store_list = list(search_sel.xpath('//h3[@class="showroom-name"]/a/@href'))
    ...
    for store in store_list:
            ...
            filter(
                ...
                [
                    ...
                    for x in store_sel.xpath(
                        '//div[contains(@class,"info")]//div[contains(@class,"address")]//text()'
                    )
                ],
            )
        )
        ...
        city = " ".join(full_address[1:2]).strip().strip(", ").strip()
        state = " ".join(full_address[2:3]).strip()
        zip = " ".join(full_address[3:4]).strip()
        country_code = "US"
            ...
            list(
                filter(
                    ...
                    [
                        x.strip()
                        for x in store_sel.xpath(
                            '//div[contains(@class,"info")]//div[contains(@class,"phone")]//text()'
                        )
                    ],
                )
            )
        ...
        hours = list(
            filter(
                ...
                [
                    x.strip()
                    for x in store_sel.xpath(
                        '//h4[text()="Showroom Hours"]/following-sibling::p//text()'
                    )
                ],
            )
        )
