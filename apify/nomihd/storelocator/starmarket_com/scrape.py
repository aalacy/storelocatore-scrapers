...
website = "starmarket.com"
domain = "https://local.starmarket.com/"
...
def get_store_data(store_sel, page_url):
    ...
    location_name = " ".join(
        store_sel.xpath('//h1[@class="ContentBanner-h1"]//text()')
    ).strip()
    street_address = "".join(
        store_sel.xpath(
    ...
    phone = "".join(
        store_sel.xpath(
            '//div[@class="Core-phones"]' '//div[@itemprop="telephone"]/text()'
        )
    ).strip()
...
def fetch_data():
    ...
    states_sel = get_selector("https://local.starmarket.com/")
