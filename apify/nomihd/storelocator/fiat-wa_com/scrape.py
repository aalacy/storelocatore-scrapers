...
def fetch_data():
    # Your scraper here
    search_url = "https://fiat-wa.com/en/find-a-dealer/"
    with SgRequests() as session:
        search_res = session.get(search_url, headers=headers)
        search_sel = lxml.html.fromstring(search_res.text)
        countries_dict = {}
        areas = search_sel.xpath('//select[@name="devis_conce"]/option[position()>1]')
        for area in areas:
        ...
        for store in stores:
            ...
            if len(location_name) > 1:
                temp_addresses = ", ".join(store_info_list).strip().split("Location")
