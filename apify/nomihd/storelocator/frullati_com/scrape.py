...
def fetch_data():
    ...
    locations_resp = session.get(
        "https://locator.kahalamgmt.com/locator/index.php?brand=5",
        headers=headers,
    )
    ...
    for index in range(1, len(stores)):
        ...
        page_url = "https://www.frullati.com/stores/" + store_number
        store_req = session.get(page_url, headers=headers)
        store_resp = lxml.html.fromstring(store_req.text)

        hours_of_operation = " ".join(
            store_resp.xpath('//div[@class="group hours"]' "/ul/li/text()")
        ).strip()
