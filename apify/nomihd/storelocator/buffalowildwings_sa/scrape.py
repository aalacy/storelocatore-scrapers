...
def fetch_data():
    # Your scraper here
    search_url = "http://buffalowildwings.sa/store-locations.html"
    ...
    for store in stores:
        ...
        raw_address = (
            ...
            .strip()
            .replace(", Saudi Arabia", "")
            .strip()
            .split(",")
        )
        street_address = ", ".join(raw_address[:-1]).strip()
        city_zip = raw_address[-1]
        city = city_zip.rsplit(" ", 1)[0].strip()
        state = "<MISSING>"
        zip = city_zip.rsplit(" ", 1)[-1].strip()
        country_code = "SA"
