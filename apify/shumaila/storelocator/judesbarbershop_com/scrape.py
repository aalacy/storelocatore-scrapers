...
def fetch_data():    
    ...
    divlist = soup.find('ul',{'id':'nv-primary-navigation-main'}).findAll('ul',{'class':'sub-menu'})[1].findAll('li', {'class': "menu-item"})   
    p = 0
    for div in divlist:
        ...
        try:
            loclist = r.text.split('"@context": "https://schema.org",',1)[1].split('</script>',1)[0]
        except:
            continue
        loclist ='{'+loclist
