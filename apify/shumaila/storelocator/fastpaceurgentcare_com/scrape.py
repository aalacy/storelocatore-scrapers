...
def fetch_data():
    ...
    while True:
        ...
        link = 'https://www.fastpacehealth.com'+soup[start:end].replace("\\",'').replace('"','').replace(' ','-')     
        start = end + 1
        ...
        if address.find('COMING SOON') == -1:
            ...
            hours = soup1[mstart:mend].replace("\\",'').replace('"','').replace(',',', ')  
           
            if len(hours) < 3:
                hours = '<MISSING>'
        
            data.append(['https://fastpaceurgentcare.com/',
                            link,                   
                            title,
                            street.lstrip(),
                            city.lstrip(),
