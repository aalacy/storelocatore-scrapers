        ...
        for loc in loclist:
            ...
            if len(loc['slug'].strip()) > 5:
                ...
                if link in checklist:
                    ...
                    try:
                        hourslist = '['+r.text.split('"openingHoursSpecification":[',1)[1].split('],',1)[0]+']'
                        hourslist = json.loads(hourslist)
                    except:
                        try:
                            hourslist = '['+r.text.split('"openingHoursSpecification":[',1)[1].split('</script>',1)[0].strip()
                    ...
                    for hr in hourslist:
                        try:
                            for day in hr['dayOfWeek']:
                                hours = hours+day+' '+hr['opens']+'-'+hr['closes']+' '
                        except:
                            pass
