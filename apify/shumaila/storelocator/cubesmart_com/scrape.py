...
def fetch_data():
    ...
    state_list = soup.findAll('h2')
    print(len(state_list))
    #driver.quit()
    for li in state_list:
        statelink = li.find('a')
        statelink = "https://www.cubesmart.com" + statelink['href']
        #statelink = "https://www.cubesmart.com/district-of-columbia-self-storage/"
        print(statelink)
        driver1 = get_driver()
        driver1.get(statelink)
        time.sleep(1)
        soup1 = BeautifulSoup(driver1.page_source, "html.parser")
        #print(soup1)
        #p = 1
        city_list = soup1.findAll('a',{'class':'csUnitButton csGreenButton'})
