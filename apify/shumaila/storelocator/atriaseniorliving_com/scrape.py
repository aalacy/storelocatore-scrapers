...
def fetch_data():
    ...
    loclist = r.text.split('communityData:',1)[1].split(',communityDataSearch',1)[0].replace('\"','').replace('{','{"').replace(':','":"').replace(',','","').replace('}','"}').replace('}","{','},{').replace('\\u002F','\\').replace('https":"\\','https:\\').replace('\\',"//").replace(']"',']').replace(':"[',':[')
    ...
    for loc in loclist:
        ...
        try:
            if len(longt) < 3:
                longt = r.text.split(',"ListItem","'+str(lat)+'","',1)[1].split('"',1)[0]
        except:
            lat = longt = '<MISSING>'
        if len(str(store)) < 3:
            store = '<MISSING>'
