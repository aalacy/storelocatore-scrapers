...
def fetch_data():
    ...
    for zip_code in mylist:
        ...
        url='https://locations.fivebelow.com/index.html?q='+str(zip_code)+'&qp='+str(zip_code)+'&l=en'
        ...
        for loc in loclist:
            ...
            street = loc['address']['line1']+' '+str(loc['address']['line2'])+' '+str(loc['address']['line3'])
            ...
            try:
                ...
                for hr in hourslist:
                    ...
                    if str(hr['isClosed']) != 'False':
                        ...
                        start = str(hr['intervals'][0]['start'])[0:2]+':'+str(hr['intervals'][0]['start'])[2:]+' am '
                        endd = int(str(hr['intervals'][0]['end'])[0:2])
                        if endd > 12:
                            endd = endd -12
                        endstr = str(endd) + ':' + str(hr['intervals'][0]['end'])[2:] + ' pm '
...
def scrape():
    results = fetch_data()
    with SgWriter(deduper=SgRecordDeduper(SgRecordID({SgRecord.Headers.STREET_ADDRESS}), duplicate_streak_failure_factor=-1)) as writer:
        results = fetch_data()
        for rec in results:
