...
WebDriverWait(driver, 40).until(EC.element_to_be_clickable((By.XPATH, "//div[@id='start-geocoder']/div[2]/input[@type='text']"))).click()
logger.info('___________Sleeping for a few seconds_______________')

driver.find_element_by_xpath("//div[@id='start-geocoder']/div[2]/input[@type='text']").send_keys("alaska")
time.sleep(randint(8, 10))

# Wait until search button becomes visible
WebDriverWait(driver, 40).until(EC.element_to_be_clickable((By.XPATH, "//div[@id='start-geocoder']/div[2]/div/span[2]/button[@class='btn-search']"))).click()
...
all_urls = raw_data.xpath('//div[@id="location-list"]/ul/li/div/div/div/div/strong/a/@href')
...
def fetch_data():
    ...
    for url_store in all_urls:
        url_store = url_flooring_base + url_store if url_flooring_base not in url_store else url_store
        ...
        street_address = ' '.join(street_address.split()) if street_address else "<MISSING>"
