...
logger = SgLogSetup().get_logger("clubmonaco_com_hash_se")
MAX_WORKERS = 10
...
def fetch_data(sgw: SgWriter):
    ...
    # SWEDEN
    search_se = DynamicGeoSearch(
        country_codes=[SearchableCountries.SWEDEN],
        expected_search_radius_miles=50,
        granularity=Grain_8(),
        use_state=False,
    )
    country_se = search_se.current_country().upper()

    sweden_custom_latlng = [(59.334363, 18.0679653)]

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        tasks = []
        # Task - Sweden
        task_se = [
            executor.submit(fetch_records, latlng, search_se, country_se, sgw)
            for latlng in search_se
        ]
        tasks.extend(task_se)

        # Task - Sweden
        task2 = [
            executor.submit(fetch_records, latlng, search_se, country_se, sgw)
            for latlng in sweden_custom_latlng
        ]
        tasks.extend(task2)
