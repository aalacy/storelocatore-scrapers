...
def get_store_urls_and_latlng(http):
    store_urls_latlng = []
    total = 0
    for latlng_num, item in enumerate(CANADA_LATLONG_LIST[0:]):
        lat = item.split(",")[0]
        lng = item.split(",")[1]
        url = (
        ...
        for slul in store_list_ul:
            ...
            for idx, page_url_rel in enumerate(page_urls_rel):
                page_url = f"{BASE_URL}{page_url_rel}"
...
def fetch_records(http: SgRequests):
    list_of_store_urls_latlng = get_store_urls_and_latlng(http)
    logger.info(f"Total # of Store URLs: {len(list_of_store_urls_latlng)}")
    list_of_store_urls_latlng = list(dict.fromkeys(list_of_store_urls_latlng))
    logger.info(f"Total # of Unique Store URLs: {len(list_of_store_urls_latlng)}")
    for idx, store_url_latlng in enumerate(list_of_store_urls_latlng[0:]):
        logger.info(f"[{idx}] Pulling the data from: {store_url_latlng}")
        logger.info(
            f"Scraping Store Number: {idx} out of {len(list_of_store_urls_latlng)}"
        )
        store_url = store_url_latlng[0]
        r_store = http.get(store_url, headers=HEADERS)
        sleep(randint(1, 4))
        sel = html.fromstring(r_store.text, "lxml")

        # Locator Domain
        locator_domain = "ultramar.ca"
        ...
        # Location Name
        xpath_location_name = '//*[@class="station__title"]/text()'
        location_name = sel.xpath(xpath_location_name)
        location_name = "".join(location_name).strip() if location_name else MISSING
        logger.info(f"[{idx}] Location Name: {location_name}")

        # Street Address
        street_address = "".join(sel.xpath('//span[@itemprop="streetAddress"]/text()'))
        street_address = street_address if street_address else MISSING
        logger.info(f"[{idx}] Street Address: {street_address}")

        # City
        city = "".join(sel.xpath('//span[@itemprop="addressLocality"]/text()'))
        city = city if city else MISSING
        logger.info(f"[{idx}] City: {city}")

        # State
        state = "".join(sel.xpath('//span[@itemprop="addressRegion"]/text()'))
        ...
        # Location Type
        location_type_data = get_station_location_type(sel, store_url)
        ...
        try:
            latitude = store_url_latlng[1]
            latitude = latitude if latitude else MISSING
        except:
            latitude = MISSING

        try:
            longitude = store_url_latlng[2]
        ...
        hoo = sel.xpath(xpath_hoo1)
        hoo = "".join(hoo).strip()
        logger.info(f"[{idx}] Hours of Operation Raw Data1: {hoo}")

        xpath_hoo2 = (
            '//div[img[@alt="24h clock"]]/div[@class="station__hours"]/div/span/text()'
        )
        hoo2 = sel.xpath(xpath_hoo2)
        ...
        if "Express Mart" not in location_type:
            continue
        raw_address = MISSING
        yield SgRecord(
