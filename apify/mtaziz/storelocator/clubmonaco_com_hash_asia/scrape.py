...
def fetch_data(sgw: SgWriter):
    ...
    # CHINA
    search_ch = DynamicGeoSearch(
        country_codes=[SearchableCountries.CHINA],
        expected_search_radius_miles=300,
        granularity=Grain_8(),
        use_state=False,
    )
    country_ch = search_ch.current_country().upper()

    # HONG_KONG
    search_hk = DynamicGeoSearch(
        country_codes=[SearchableCountries.HONG_KONG],
        expected_search_radius_miles=300,
        granularity=Grain_8(),
        use_state=False,
    )
    country_hk = search_hk.current_country().upper()

    # KOREA_S
    search_kr = DynamicGeoSearch(
        country_codes=[SearchableCountries.KOREA_S],
        expected_search_radius_miles=300,
        granularity=Grain_8(),
        use_state=False,
    )
    country_kr = search_kr.current_country().upper()

    # MACAO
    search_mo = DynamicGeoSearch(
        country_codes=[SearchableCountries.MACAO],
        expected_search_radius_miles=500,
        granularity=Grain_8(),
        use_state=False,
    )
    country_mo = search_mo.current_country().upper()

    # SINGAPORE
    search_sg = DynamicGeoSearch(
        country_codes=[SearchableCountries.SINGAPORE],
        expected_search_radius_miles=500,
        granularity=Grain_8(),
        use_state=False,
    )
    country_sg = search_sg.current_country().upper()

    # TAIWAN
    search_tw = DynamicGeoSearch(
        country_codes=[SearchableCountries.TAIWAN],
        expected_search_radius_miles=500,
        granularity=Grain_8(),
        use_state=False,
    )
    country_tw = search_tw.current_country().upper()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        # Task - China
        task_ch = [
            executor.submit(fetch_records, latlng, search_ch, country_ch, sgw)
            for latlng in search_ch
        ]
        tasks.extend(task_ch)

        # Task - Hong Kong
        task_hk = [
            executor.submit(fetch_records, latlng, search_hk, country_hk, sgw)
            for latlng in search_hk
        ]
        tasks.extend(task_hk)

        # Task - Korea
        task_kr = [
            executor.submit(fetch_records, latlng, search_kr, country_kr, sgw)
            for latlng in search_kr
        ]
        tasks.extend(task_kr)

        # Task - Macao
        task_mo = [
            executor.submit(fetch_records, latlng, search_mo, country_mo, sgw)
            for latlng in search_mo
        ]
        tasks.extend(task_mo)

        # Task - Singapore
        task_sg = [
            executor.submit(fetch_records, latlng, search_sg, country_sg, sgw)
            for latlng in search_sg
        ]
        tasks.extend(task_sg)

        # Task - Taiwan
        task_tw = [
            executor.submit(fetch_records, latlng, search_tw, country_tw, sgw)
            for latlng in search_tw
        ]
        tasks.extend(task_tw)
