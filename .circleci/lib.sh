...
get_updated_python_files() {
	list_updated_files | grep -E '[^.]+\.py$' | tr '\n' ' '
}

get_updated_node_files() {
	list_updated_files | grep -E '[^.]+\.jsx?$' | tr '\n' ' '
}

check_update_scope() {
	crawler_regex='apify(\/[^/]+){3}'
	updated_crawlers=$(list_updated_files | xargs -I{} dirname {} | sort | uniq | grep -E "$crawler_regex")
	updated_non_crawler_files=$(list_updated_files | grep -E -v "$crawler_regex")
	status_code=0
